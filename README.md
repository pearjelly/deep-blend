...
预期：**16 个套件、64 个文件**全部通过。其中契约层（`run.mjs`，不需要 Blender）是
**49 个文件 = 1142 项自计断言（21 个文件打印计数）+ 271 个 `node:test` 用例（28 个文件）**。
...
它是多少，而一个「为了数其它套件而跑其它套件」的测试会让整套的成本翻倍。所以 1142 和 271 是
...
总数直接相减，会凭空多出或少掉两项。上面那个 1142 是**装了** preset 的读数，没装是 1140。
单跑某一层：
