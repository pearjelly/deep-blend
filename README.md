...
预期：**16 个套件、46 个文件**全部通过。其中契约层（`run.mjs`，不需要 Blender）是
**31 个文件 = 830 项自计断言（12 个文件打印计数）+ 202 个 `node:test` 用例（19 个文件）**。
...
它是多少，而一个「为了数其它套件而跑其它套件」的测试会让整套的成本翻倍。所以 830 和 202 是
