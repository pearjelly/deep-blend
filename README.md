...
预期：**16 个套件、52 个文件**全部通过。其中契约层（`run.mjs`，不需要 Blender）是
**37 个文件 = 848 项自计断言（12 个文件打印计数）+ 249 个 `node:test` 用例（25 个文件）**。
...
它是多少，而一个「为了数其它套件而跑其它套件」的测试会让整套的成本翻倍。所以 848 和 249 是
