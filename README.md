...
**57 个文件 = 1315 项自计断言（28 个文件打印计数）+ 278 个 `node:test` 用例（29 个文件）**。
...
它是多少，而一个「为了数其它套件而跑其它套件」的测试会让整套的成本翻倍。所以 1315 和 278 是
...
总数直接相减，会凭空多出或少掉两项。上面那个 1315 是**装了** preset 的读数；同一份快照在没有装
