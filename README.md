...
| **Python 3** | 只有一处：`contract/render-job.test.mjs` 用普通 CPython 跑 `deepblend_util.py`，比对两边的帧命名 | 那**一条**失败并说清缺什么，其余 78 条照跑 |
...
预期：**16 个套件、77 个文件**全部通过。其中契约层（`run.mjs`，不需要 Blender）是
**62 个文件 = 1487 项自计断言（32 个文件打印计数）+ 302 个 `node:test` 用例（30 个文件）**。
...
它是多少，而一个「为了数其它套件而跑其它套件」的测试会让整套的成本翻倍。所以 1487 和 302 是
...
总数直接相减，会凭空多出或少掉两项。上面那个 1469 是**装了** preset 的读数；同一份快照在没有装
