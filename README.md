...
| **Python 3** | 只有一处：`contract/render-job.test.mjs` 用普通 CPython 跑 `deepblend_util.py`，比对两边的帧命名 | 那**一条**失败并说清缺什么，其余 81 条照跑 |
...
**65 个文件 = 1489 项自计断言（32 个文件打印计数）+ 337 个 `node:test` 用例（33 个文件）**。
