...
| 日期 | 里程碑 | 变更 |
|---|---|---|
| 2026-09-22 | 商业就绪（常驻目标） | D200：**SPEC §21.1 的最后一条被规格的所有者显式修订——自动前进是允许的，自动跳步不是。** 触发它的事实是一份用户下达的常驻目标（「持续完善 deep-blend 的功能与细节，直到它达到可商用水平」），它的执行方式是**跨多轮自动继续**，而 SPEC §21.1 的第 12 步写着「停止，不自动进入下一里程碑」。§21.1 要防的是两件事，**那两件继续有效，而且更严**：① 一个里程碑没做完就跳到下一个——**仍然禁止**；② 一次会话横跨多个未完成里程碑——**仍然禁止**。被取消的只有「做完之后必须停下等人」这一条。**为什么这条修订本身必须先落盘**：不落盘的话，之后每一轮都在违反一份没被改过的规格，而违反它的证据（提交历史）会与合法工作长得一模一样——**一份没有被写下来的授权，事后无法与「我自己决定继续」区分开**。执行上的两条硬约束由此而来：**一次只开一条线**（不许在功能半开时另起一条），以及**每一轮的进展必须可数**——「可数」是「自动前进」这一侧唯一的刹车；账本是 `deepblend/docs/commercial-readiness.md`，盯着它的是 `deepblend/tests/contract/commercial-readiness.test.mjs` |
| 2026-09-23 | 商业就绪（产品文案的语言） | D201：**工作台跟随部署的 locale，回退英文——这不是偏好，是平台自己的契约。** 触发它的是账本 C16（「产品文案只有中文，商业用户面是否需要双语」），而它此前**连要不要做都没决定**。量下来的事实让这个问题不再需要问人：harness 自己带 `dsh-client-locale`，它支持两个 locale（`zh` / `en`）并把回退规则写成一句话——**「English is both the locale the UI opens in when the browser names no registered language (and for non-browser runs), and the dictionary consulted after the active locale misses a key ... because a browser naming no registered language is the reader least likely to read Chinese」**（`FALLBACK_LOCALE = "en"`）；而当前 locale 由它写在页面上（`document.documentElement.lang`）。DeepBlend 的工作台此前在**每一处**用户可见位置硬编码中文，于是**平台特意路由到英文的那个读者，看到的是一整屏中文**——这正是那条规则存在的理由。修法：把 138 条文案收进一个带 `#region strings` 标记的表（`zh` / `en` 两侧**键集相同**，这是 harness 自己的不变量），locale 从页面读、回退英文，`t(key, params)` 支持占位符；断言 `contract/workbench-copy.test.mjs` 盯四件事：两侧键集相同、回退确实是英文（未注册语言与无浏览器两种情形）、**表外不许再有任何中文文案**（否则表只是摆设）、以及每个 `t()` 调用点都指向存在的键（**双向**：用了不存在的键与表里没人用的键都红）。**为什么记进登记册**：它定下的是产品面的语言策略——**跟随平台，不自造偏好**；以后新增任何用户可见文案都进那张表，而「表外不许有中文」这条断言就是它的执行者 |
...
| 2026-09-22 | M6（发布链） | D199：**三条路线的同一性只能装一次读回来，而且读的东西不能只有版本号。** 触发它的事实是两条路线的读数：`/releases/latest/download/deepblend-bundle.tgz` 与 `@deepblend/dsh-blender-bundle` 都服务 **0.1.0**——一个在 M6 之前构建的产物——而 `main` 上 `/deepblend/workbench` 返回 200。三条路线声称同一个产品，两条在说旧的那个，**而没有任何一条命令会因此变红**。探针此前只轮询 `/deepblend/capabilities`，那是一条**从 M0 就存在**的路由，所以它在三个版本上都 200——「路由装上了」与「装的是仓库当前那一版」是两件事，而只有前者被测过。**修法**：探针加两条读回，判据对三条路线完全相同：`installed version`（从 profile 里那个 bundle 的 manifest 读，连同它钉的六个精确版本）与 `workbench route`（`GET /deepblend/workbench`——**只有当前版本才有的路由**；陈旧产物在那里只能回答 404，而它的版本号是**诚实地**旧的）。顺带修掉探针自己的 `result:` 行：它此前只按「用没用 `--spec`」写结论，于是在一次 `dsh plugin add` 退出 1、capabilities 404、presets 一个没装的运行之后，仍然打印「the spec installs, serves DeepBlend and delivers both presets」——**一份被当作证据提交的日志，其结论必须由读数推导，不能由模式推导**；现在结论与退出码都从同一张 problems 表来。**不这样做**：发版之后没人能说出「发出去的是不是 main」，而三条路线会继续各自声称一个版本。实测：三条路线（隔离 store）全部 `installed version: …@0.2.0` + `workbench route: HTTP 200, text/html` + 两个 preset 字节一致 + `problem: null` |
| 2026-09-22 | M6（发布链） | D198：**版本号有且只有一个来源，而「陈旧」这条门禁不能进契约层。** 触发它的事实是 8 个 manifest 各写各的 `0.1.0`，没有任何地方规定它们必须一致——「它们现在一致」是观察不是约束（D38 的形状）。代价在交付形态上：bundle 把六个同级包钉在精确版本上，而 tarball **装着**它们，所以一次只升到 6/7 的升版会产出一个「npm 路线安装 404、tarball 路线自相矛盾」的产物，**而两次构建都成功**。**修法**：`deepblend/version.json` 是源，`tools/release-version.mjs --check` 是那条断言（8 个 manifest 必须等于源；仓库根 manifest 也在集合里，因为「允许不同的第二个数字」正是这条策略要消掉的东西），`--sync` 让「手改六个漏一个」不可能发生。**第二条判断是门禁放在哪里**：`--stale`（最新 Release 的 `packages/` 树与**工作树**是否同一份）**不进契约层**——它在引入它的那一次提交上就是红的（M6 改了 `packages/**` 而没有 Release 带上它），而一条注定要红的断言放进契约层，会把「契约层全绿」变成一句假话。它属于发布那一族：让陈旧可见，而不是让主套件变红。**两次变异各暴露一个洞**：第一版 `--stale` 用 `git diff tag..HEAD`，于是**未提交**的 `packages/**` 改动读出来是 `changed: 0` + `FRESH`（那正是操作者发版前所处的状态），改为与工作树比较、并把未跟踪文件算进来；第一版「不许有第二份版本号」只 `readdirSync('deepblend')` 而没有下到 `tools/`，于是往 `blender-release.json` 里加一个 `version: 0.2.0` **活了下来**——现在改成「仓库里每个 package.json 都在 lockstep 集合里」+「工具读的 JSON 里不许再出现这个数字」两条，两次变异都红 |
| 2026-09-20 | M5（卸载路径） | D197：`install.md` §6 的卸载四步**第一步只是改配置**——`plugin:install --portable` 把 operator layer 清成 `[]`（它自己的注释就这么说），**但 bundle 仍被 profile 组合**，所以插件还装着，`dsh` 启动照样挂三行。顺着量出**第二个**缺陷：生态自己的卸载命令 `dsh plugin remove …` 以 `ERR_PNPM_CANNOT_REMOVE_MISSING_DEPS` 失败，因为 `install-plugin.mjs` 只写 `dsh.profile.bundles`（Loader 组合需要的全部）而不写 `dependencies`（pnpm 与 `plugin remove` 读的那个）——而生态的 `plugin add` **两个键都写**，两条安装路径产出的状态不一致。**修法**：安装器写两个键（`link:<仓库里的 bundle>`，与 `plugin add <路径>` 一致），并作为**独立的 drift 检查**（`plugin:check` 报缺、`--check` 退 1、安装补上）；手册先给卸载命令并写明以前为何不行。**不这样做**：操作者按手册走完以为卸载了，插件仍在每个会话里挂载三行——而唯一的官方卸载命令还报一个关于 pnpm 的错误，读者无从知道该改哪个键。实测：真实 home 先报 drift → 一次安装补上 → `plugin remove` exit 0 且 `bundles: []`；用例跑完整往返，变异「不写 dependency」红 |
| 2026-09-20 | M5（可安装包的清单字段） | D196：清单的字段表逐条对照可安装包，发现两处缺口：**`dsh.client` 声明在 `ui` 依赖上**——该字段描述的是**被安装的那个包**，安装器不读依赖里的它，于是生态路径装出来的是**没有工作台的宿主**（本仓库自己走 operator layer，所以本地一切正常，这也是它没被注意到的原因）；**`files` 完全没有**——要求是包含 patch 与所有运行时入口，漏了 patch 就是「装了但什么都不挂载」。**修法**：bundle 的 `dsh` 两个键都有（与生态参考清单一致），`files` 列出 `lib`/`cordis.patch.yml`/`screenshots.json`。**不这样做**：评审与用户都会看到一个装上了却不做事的插件。实测 `plugin:check` exit 0、契约层 62/62（operator layer 不受影响）；检查断言四条字段规则 + 一条解析规则（patch 每行的 `name:` 指向的包都在仓库里），四条变异全红 |
| 2026-09-20 | M5（插件生态的 peer 规则） | D195：生态的 contributing 明文规定官方 `@deepseek-ai/*` 必须是 `peerDependencies`，是**点名的拒绝原因**；实测 `host`/`provider-local`/`ui` 把 cordis 与 schemastery、`tool` 把 dsh-tools 声明成了 `dependencies`。**为什么是硬伤**：插件自带一份 cordis 就可能加载**与正在运行它的那个不同的** harness，契约测试会对着产品根本不加载的那份变绿。第二条规则同样要命：范围必须带**显式预发布分支**——node-semver 只在比较符与预发布版本**同 tuple 且自身带预发布标记**时放行，所以看起来更宽的 `>=0.0.1-rc.1 <0.2.0` 会**静默排除**每个 `0.1.0-rc.*`（而 dsh-tools 装的正是 `0.1.5-rc.2`）。**修法**：搬到 peerDependencies；正式版用普通范围、预发布版用文档给的显式分支形式。**不这样做**：清单直接拒收，或用户装出一个跑在另一份 harness 上的插件。**安全性来自实测**：工作区的 12 条链接由 `link-workspace.mjs` 从**源码 import** 推导（正是为了不出现第二份 harness），改完 `setup:check` 仍解析 12 个、契约层 62/62 |
| 2026-09-20 | M3（同一路径的资产条目） | D194：资产落地路径是 `assets/raw/<文件名>`——**按文件名共享，不按 asset id**。实测（同名不同字节的两个源）：两次 ingest 都成功、文件是两者之一且完好，而**清单里的哈希两个都不是**。原因不是哈希算错，而是清单对这个路径**留了两条条目**（过滤条件只有 `assetId`），第一条描述的字节已被第二条替换——而**制品索引按 `path` 去重**（第 118 轮那条注释），同一个事实在两处不一致。**修法**：过滤时把 `path` 也算上，并让落地走「临时名 + rename」。**不这样做**：来源记录对同一个文件说两套话，审计者无法判断磁盘上那份是谁。这一改立刻让第 108 轮那条老用例变红——**它把缺陷钉住了**，现在断言诚实的性质；变异「只按 id 过滤」红，「原地写回」是**等价变异**（这一段全程同步，两次调用不可能交错）——`renameSync` 因此是**防御性**的，理由写在代码处 |
| 2026-09-20 | M3（一个 job 一个编码器） | D193：`encodedPath` 是**一个 job 一个路径**，而导出不是瞬时的（shell 出去跑 ffmpeg 再探结果），所以第二次导出会在第一次还在编码时写同一个文件，各自去验证一个对方还在写的文件。实测：两个并发 `_deliverJob({reason:'export'})` **都失败**于 `ENCODE_VERIFY_FAILED`，而那条消息**怪罪视频的属性**——真正的成因是碰撞。**修法**：按 `projectId/jobId` 的**在飞表**，第二个调用以新码 `EXPORT_IN_PROGRESS` 被拒并点名是谁在编、编到哪个文件；守卫包住整个方法体，`finally` 里一定清掉。**不这样做**：两个编码器把同一个 MP4 写成谁都无法验证的东西，而报错指向错误的方向。**清理是更要紧的一半**：泄漏的标志会把一次竞态变成这个 job 永久不可交付 |
| 2026-09-20 | M1/M5（声明出来的资产哈希） | D192：资产工作流是两步——`blender_asset_ingest` **算出**它写下字节的 sha256，调用者随后用 `asset.add` 把它**声明**进 SceneSpec——而两边**从来没有被对照过**：patch applier 是纯文档函数（没有文件系统，这是它该有的形状），Python 侧**根本不读** `assets[].sha256`（实测 0 次出现）。于是一份 patch 可以把**假的**来源写进场景，而交付清单与任何审计者都会相信它。**修法**：核对放在**同时握着文档和 store** 的交易层——应用 patch 后，对每个 `asset.add`，文件存在就哈希并与声明比对，不等则抛 `ASSET_HASH_MISMATCH` 并**报出两个数字**、什么都不提交。**不这样做**：来源记录变成装饰。**文件不存在则放过**（「先声明后落盘」是另一个问题，编译器会大声拒绝）——变异「连缺失也拒」红，证明这条边界是被钉住的 |
| 2026-09-20 | M3（审批的主语在时间上漂移） | D191：交付渲染的审批闸门在宿主拒绝、工具面替模型去问人、拿到 `allowed-once` 后**重发同一请求**——而两处细节让审批的主语可移动：提示词**没有 revision**（只说「1200 帧」，看不出是哪个场景的帧），而 `revision` 在 schema 里**可选**（「默认当前 revision」），所以人类读提示的那段时间里落下一个 patch，重发就渲染**另一个 revision**。**修法**：提示词带上 revision（只在宿主报了它时出现，与同一句里 range 的既有约定一致），重发时**钉住**拒绝里报的那个。**不这样做**：批准的与执行的不是同一件事，而且没有任何记录显示它们不同。实测：`requestApproval` 发给操作者的只有 `{agent, toolName, callId, reason, signal}`——没有结构化 detail，所以那句话就是渠道；三条变异全红 |
| 2026-09-20 | M5（来源记录） | D190：第 189 轮把重定向链路放进了**工具结果**（`redirectedFrom`），而这一轮问下一格：**那份会被后来的人读的记录里写的是哪个 URL**——`assets/manifest.json` 只写被批准的那个。工具结果在对话结束时就没了，于是重定向之后，来源记录答不出它存在的那个问题。**修法**：`source` 带 `url`（我要的是什么）与 `resolvedUrl`（我拿到了什么），后者**只在链路真的移动过时出现**。**不这样做**：manifest 指向一个没有回答的地址，而读它的人无从知道。两条变异红（去掉 `resolvedUrl`、把它写成批准的那个） |
| 2026-09-20 | M5（重定向绕过审批） | D189：远程资产抓取只检查**原始 URL** 的协议，然后 `fetch(parsed, { redirect: 'follow' })`——**把决定权交给网络**。于是人类批准的是他们看到的 URL，而服务器回一个 `302 Location: http://127.0.0.1:3080/…` 就把请求移到**这台机器自己的服务**上，那一跳既不在宿主的规则里、也不在操作者眼里。**审批闸门本身是真的**（它确实拦住了未批准的 URL），被绕过的是它的**主语**。**修法**：`redirect: 'manual'` **逐跳**跟随，每跳过同一条协议规则，跳数上限 5，并把链路作为结果的一部分（`redirectedFrom: {hops, chain}`）。**回环地址故意不拒**（写进注释）：审批闸门才是「取这个 URL」的控制，本地资产服务器是正当用法，测试套件自己的远程用例就是从 127.0.0.1 提供的——缺的不是禁令，而是**看见请求最终去了哪里**的能力。四条变异全红 |
| 2026-09-20 | M1（一个会静默跳过的守卫） | D188：宿主的多边形守卫读 `compileReport.sceneFingerprint.totalPolygons`，而报告由 **Python** 造。判断写成 `typeof polygons === 'number' && polygons > max`——**字段名任何一侧改名，`polygons` 就是 `undefined`，条件为假，守卫不报错、只是不运行**：一个五倍重的场景被提交，正是 D101 修掉的缺陷。**修法两半**：守卫**拒绝**「报告存在却没有该计数」（`PROTOCOL_VERSION_MISMATCH`，什么都不提交——看不见输入的检查不允许通过），并把字段名**跨语言钉住**（跑 Python 的 `scene_fingerprint`，与宿主实际读的字段对照）。**不这样做**：检查在缺失输入时通过，而它的存在正是为了防止那件事。这一改立刻暴露**四个测试桩都不带 fingerprint**——那些用例一直在走「字段缺失→跳过守卫」这条分支；四条变异全红（含「枚举的键换成值」——`code` 变成 undefined，是测试读数揭穿的） |
| 2026-09-20 | M2（同一 revision 的两个预览互删） | D187：`renderPreview` 的暂存目录是**按 revision** 的（`<revision>/.render-staging/<camera>.png`），而清理**删掉整个目录**——两个预览同时跑时**先完成的把另一个的图删掉**，另一个报 `RENDER_NO_OUTPUT`（一次成功的渲染被报成失败），制品索引里也少一条。`run-all.sh` 的并发套件测的是「两个预览在**不同 revision** 上并发」，**同一 revision 从来没测过**。**修法**：暂存**按一次渲染**（job id 在同一个同步块里铸造并写成 running 记录，所以每次调用唯一），清理只删自己那一层，再用 `rmdirSync` 尝试删父目录（**非空时拒绝**，正好是想要的判断，不需要列目录也没有竞态）。**不这样做**：一次成功的渲染被报成失败，而索引与磁盘不一致。连带把第 109 轮那条断言改到新现实（让 stub 在被告知的路径上写半写文件再抛错——那才是真实渲染器的行为，断言也更强）；三条变异全红 |
| 2026-09-20 | M5（可安装包的清单） | D186：清单的字段表逐条对照可安装包，发现两处缺口：**`dsh.client` 声明在 `ui` 依赖上**（该字段描述被安装的那个包，安装器不读依赖里的它 → 生态路径装出没有工作台的宿主），**`files` 完全没有**（要求包含 patch 与运行时入口，漏了 patch 就是装了不挂载）。**修法**：两个键都写在 bundle 上，`files` 列出 `lib`/`cordis.patch.yml`/`screenshots.json`。**不这样做**：评审与用户看到一个装上了却不做事的插件。**安全性**：本仓库自己走 operator layer，所以本地不受影响（这也是缺口没被发现的原因）；检查四条字段规则 + 一条解析规则，四条变异全红 |
| 2026-09-20 | M1/M5（跨语言的字段名对齐） | D185：宿主的多边形守卫读 `compileReport.sceneFingerprint.totalPolygons`，而报告是 **Python** 造的（`bootstrap.py` 的 `scene_fingerprint`）。判断写成 `typeof polygons === 'number' && polygons > max`——**字段名任何一侧改名，`polygons` 就是 `undefined`，条件为假，守卫不报错、只是不运行** ✗：一个五倍重的场景被提交，正是 D101 修掉的缺陷；而字段名的对齐**从来没有被断言过**（唯一提到它的是注释）。两半修法：**守卫拒绝**报告存在却没有该计数的情况（`PROTOCOL_VERSION_MISMATCH`，「两份包对协议理解不一致」，什么都不提交——**看不见输入的检查不允许通过**）；**字段名钉住**（新增跨语言断言：跑 Python 的 `scene_fingerprint`，把它的键与宿主**实际读的字段**——从源码抽 `sceneFingerprint.X`——对照；读数 `pythonEmits: [objectCounts, specSchemaVersion, totalPolygons, totalVertices]` / `hostReads: [totalPolygons]`）。**这一改动立刻暴露出四个测试桩都不带 fingerprint** ✗（`{ report: { validation: {} } }`）——那些用例一直在走「字段缺失 → 跳过守卫」这条分支，**从没碰过守卫真正运行的那一半** ✓；补上真实报告形状之后才真的覆盖它。本轮我自己也踩了一次「枚举的键 vs 值」（写成 `BlenderErrorCode.BLENDER_PROTOCOL_VERSION_MISMATCH` ✗ = 枚举的值，键是 `PROTOCOL_VERSION_MISMATCH` → `code` 变成 undefined，**是测试读数揭穿的** ✓），现在有一条变异专门钉住它。四条变异全红；契约层 61 文件 / 1460 → **1462** 项；跑新探针（`r106` 列） |
| 2026-09-19 | M2（索引的身份是 path） | D184：本轮从一个**假设**出发（同一个 round 的第二份评审会覆盖第一份、而 `manifest.reviews` 留下两条 → 「两条指向一个文件」），据此加了错误码 `VISUAL_REVIEW_EXISTS` 并在花 Blender 时间前拒绝重复轮次——**契约层全绿，而 `run-all.sh` 把 M2 套件打红了** ✗：那套件**故意**对同一 revision 的同一 round 反复评审（检查确定性：「同一缺陷每次都报、从不凭空发明」）——**重复评审是合法操作，我的拒绝是错的** ✓，于是拒绝、错误码、分类与用例**全部撤回** ✓。真相在写者自己的注释里：`recordRevisionArtifact` 写着「Re-emitting the same path replaces its entry rather than duplicating it」——**索引本来就按 path 去重**，round 是身份，最后写入的那份就是这一轮的评审 ✓。**我从字段名推断行为而没有读写入者** ✓（本会话第九次量具/推断出错，而这次是**验收套件**抓住的 ✓）。这一轮真正留下的：那条「按 path 替换」的承诺**只有注释没有断言** ✓ →补两条（同 path 替换而非追加 ✓；**同 kind 不同 path 的两条都要留住** ✓——只按 `kind` 去重的变异在第一条下活着，补上第二个视角之后才红 ✓）。两条变异全红；契约层 61 文件 / 1454 → **1456** 项；产品代码未改，读数沿用 **32 (0.3%)** |
| 2026-09-19 | M1（同一 revision 的两个预览互删） | D183：第 116 轮那把尺子（**读—改—写靠什么保证原子性**）指向 revision 的制品索引 `recordRevisionArtifact`（读 manifest → 追加 → 写回，**同步**所以安全 ✓——而它正是文档说的「会被持久化、被交付、被读的那一份」），于是写了「同一 revision 的两个预览同时发起，两条都要进索引」的用例——**当场红** ✗：`{"left":"…frame61-camera-main.png", "right":"RENDER_NO_OUTPUT: The Blender renderer reported success but wrote no image."}`。根因：`renderPreview` 的暂存目录是**按 revision** 的（`<revision>/.render-staging/<camera>.png`），而清理**删掉整个目录**——两个预览同时跑时**先完成的把另一个的图删掉**，另一个报 `RENDER_NO_OUTPUT`（一次成功的渲染被报成失败，索引里也少一条）。`run-all.sh` 的并发套件测的是「两个预览在**不同 revision** 上并发」，**同一 revision 从来没测过**。修法：暂存**按一次渲染**（job id 在同一个同步块里铸造并写成 running 记录 → 每次调用唯一 ✓，实测确认 ✓），清理只删自己那一层 ✓，再用 `rmdirSync` 尝试删父目录（**非空时拒绝** ✓，正好是想要的判断，不需要列目录也没有竞态 ✓）。连带把第 109 轮那条断言改到新的现实 ✓（原先是**手工**在旧位置造半写文件 ✗；改成**让 stub 在被告知的路径上写半写文件再抛错** ✓——这才是真实渲染器的行为，而且断言更强：既证明写进去过，又证明被清掉）。三条变异全红（含「改回按 revision」= 原缺陷）；契约层 61 文件 / 1453 → **1454** 项；跑新探针（`r104` 列） |
| 2026-09-19 | M3（状态词的第四份拷贝） | D182：用 D181 那把尺子（**这个事实有几份拷贝**）量渲染任务状态词，发现五处：权威表 `RENDER_JOB_STATUSES` ✓、终止表 ✓（注释写清它**不是**「没有出边」——`failed` 可被显式续渲 ✓）、`UNFINISHED_STATUSES`（**手写的第四份** ✗，而它正好等于「权威表减终止表」✓）、浏览器包里的 `statusTone`（第五份 ✗）、`describeJobForHuman` 的 `case`（必要，不是重复 ✓）。手写那一份的后果**不是美观问题**：`UNFINISHED_STATUSES` 决定「一个项目是否已有交付渲染在跑」（`_activeRenderJob`），少一个状态就意味着**交付名额被交给第二个渲染器**、两个进程往同一个帧目录里写 → 改成**推导** ✓ 并加断言钉住 ✓（**变异「改回手写列表」在加断言之前是活的** ✓，实测）。顺手抓出一条**不可能失败**的检查：`ui-api.test.mjs` 那条「每个状态都有一句人话」手打七个状态、判据 `.length > 8`——而**兜底分支**（`"weird：0/0 帧"`）也长于 8 ✓✓，**把每个 `case` 都删掉它照样绿**；改成由 `RENDER_JOB_STATUSES` 驱动并要求那句**不是兜底**（兜底以 `<status>：` 开头）→ 删一个 `case` 就红 ✓。浏览器包那一份**只能源码级钉住**（自注册 CJS 工厂、无顶层 import、唯一 `require` 是 React → 拿不到 contracts ✓），与第 109 轮不可达分支同一结论，且写成**相等**而非子集（客户端自己发明的状态也会红 ✓）。三条变异全红；契约层 61 文件 / 1450 → **1452** 项；跑新探针（`r102` 列） |
| 2026-09-19 | M1（词汇表的三份拷贝） | D181：并行贡献者提交了 `material.texture.set`（schema ✓ + `scene-patch.js` 实现 ✓），而**我的**检查是绿的 ✗——第 94 轮那条「手册的操作表 == `SCENE_OPERATION_NAMES`」把手册钉在了**常量**上，而**常量自己从来没被钉在 schema 上**，于是两份拷贝一致地少了同一个操作 ✓✓（「两面只测一面」的另一种形态；schema 才是调用者真正被允许发的东西）。新增 schema↔常量绑定后立刻红（`inSchemaOnly: ["material.texture.set"]`），顺着修下去暴露出**三份拷贝**：常量 23 → 24（按本文件约定**追加末尾**——前 21 个的顺序被手册与工具描述引用着，插中间会挪动断言里每个下标）、`usage.md` 表 23 → 24、以及**模型真正读到的那一份**——工具描述里的 `OPERATION_SUMMARY` **22** → 24 ✗✗（少了 `asset.add`/`asset.remove`/纹理操作，且**没有任何检查看着它**）：**一个只能通过 `blender_scene_patch` 改场景的模型，读到的语言比它被允许说的少两种**，而它从没被告知的那两种恰好是 `blender_asset_ingest` 自己的回答让它去用的。于是加第三条绑定：工具描述必须概述每个操作、且不能发明。**已实现的那一半有断言、没实现的那一半有编号**：该操作只做了一半（schema 接受 ✓、patch 写入 SceneSpec ✓、场景摘要变化因而**会触发重渲判定** ✓，但 `deepblend_scene.py` 里这个名字出现 **0 次** ✗ → 今天设纹理渲染出的像素一模一样）；按 D100 记 **§7 #14** ✓，测试里四条断言钉住已实现的一半（写入 ✓、scene digest 变化 ✓、`changedPath` 与摘要 ✓、`texture: null` **删键**而非存 null ✓），而**替别人实现另一半**（节点图形状是他们的设计）会撞车，所以只把它说清楚。顺手修掉一条**自己造成的误报**：`security-controls` 用关键词筛 §7 的行，光秃秃的「纹理」把 `material.texture`（SPEC §5.2 的能力缺口、与安全无关）判成安全偏差 → 收窄为「纹理**尺寸**」✓（**检查太松会把正确记录判成错误**）。三条变异分别打在三份拷贝上，全红；契约层 61 文件 / 1444 → **1450** 项；跑新探针（`r101` 列） |
| 2026-09-14 | M3（编译出来的场景没人删） | D180：顺着 D179 那根线把 host 里 8 处 `removeTree(` 全查了一遍——两处「编译出来的场景」清理分别属于预览与多视角（各一个 `finally`），而**交付路径那处 `checkpoint.compiled` 属于 `_renderViewPlan`，不是交付** ✗。给交付写断言时**第一版挂错了地方**（挂在一个 revision 有 checkpoint 的用例上，那里根本不编译，断言空过 ✗——正是同一轮我自己在另一个文件里写的防空过守卫要防的事），挪到真正会编译的用例上后读数立刻是 `{"compiles":1,"status":"failed","leftovers":["render-r0002-…"]}`：**交付渲染编译出来的场景从来没有被删过** ✗。**但「直接删掉」是错的**：`resumeRenderJob` 会用 `record.checkpointPath`，删了它「继续这个渲染」就变成「渲染器打不开场景」✗——正确规则是 **`completed` 是唯一不能再续的状态**：之前编译产物是**活状态**，之后是垃圾，于是清理放在 `_driveRender` 的 `finally` 里且**只在已 completed 时**执行。三条断言（失败保留 ✓ + 续渲复用不重编译 ✓ + 完成即删 ✓）与一条「只准删暂存」的守卫（交给它 revision 的 checkpoint 必须原样留下 ✓；「长得像暂存但不在 tmp 下」的目录也必须留下 ✓——两条守卫各自需要自己的输入，只去掉 tmp 那条时别的用例全绿，补上「像暂存但不是」的输入才红 ✓）。五条变异四条红，一条是**等价变异**（只去掉名字守卫，所有输入行为不变）——「变异要带上下文瞄准」的又一例。契约层 61 文件 / 1434 → **1442** 项；跑新探针（`r100` 列） |
| 2026-09-14 | M5（成功路径上的暂存泄漏） | D179：第 106 轮的「两面都测」习惯继续用，这一轮问「两个远程导入同时在飞会怎样」——远程导入要 `await` 网络，两次调用真的会交错并走到 `assets/manifest.json` 的读—改—写；读（2090 行）与写（2105 行）之间**没有 `await`** ✓，所以这一对是安全的——**但这是一条性质而不是巧合** ✓：在那里插一个 `await`（比如把哈希改成异步）就会让一条记录静默丢失，而文件里没有别的东西会注意到 ✓。写完这条并发用例，同文件里**两条旧断言红了** ✗——那条断言的名字是对的（「每一次**失败**的抓取的暂存空间都清干净了」），但它从来没被问过**成功**的那一半 ✓：顺着量下去，**每一次成功的远程导入都会在 `<workspace>/tmp/asset-<uuid>/` 留下刚下载的字节** ✗（已在 `assets/raw/` 有一份，于是每个远程资产占两份空间且无限增长；失败路径的清理一直在，成功路径根本没有）。修法：把导入尾部包进 `try/finally`，`finally` 只清 `fetchedScratch`——该变量**只对远程来源**赋值，本地来源是调用者自己的文件，删它就是删用户数据。两条方向相反的断言：成功的远程导入不留暂存 ✓、本地来源永远不被删（文件还在且字节没变）✓。M1（关掉清理）红在第一条；M3（让本地来源也进清理）红在第二条。产品代码改了，所以跑了一次新探针（`r99` 列）；契约层 61 文件 / 1431 → **1434** 项（`host-asset-ingest.test.mjs` 21 → 26） |
| 2026-09-14 | M5（手册的表与工具的定义） | D178：`tool-contracts.md` §3 的参数表与工具定义是同一个表面的两份描述，只有一份能悄悄错——工具加了参数手册看不出来（模型读 schema），手册留着已不存在的行则读起来像能力。**第一版量具读错了**：只取每节的**第一张**表，于是报 `scene_patch` 少 `actor`/`stage`、`preview_render` 少 `width`/`height` ✗——那四个都在同一节的**第二张**表里；取全部表之后**五张表与定义逐字相符** ✓（本会话第四次「量具先出错」✓）。解析 `defineTool` 块的代码原本长在 `tool-parameters.test.mjs` 里，新检查需要同样的事实——**解析器的副本是最坏的一种副本**（两份静默漂移，而「什么都没解析到」与「通过」长得一模一样），于是抽成 `tests/lib/tool-definitions.mjs` 供两个检查共用。新增检查：**§3 每张参数表必须等于该工具声明的参数集合**（只比有表的节；至少 5 张表否则近乎空过）。变异：手册多写一个不存在的参数 → 红；共用解析器被改坏 → 红；**给 `blender_job_status` 加参数 → 不红，且这是对的** ✓——那一节用散文描述、§3 只展开契约微妙的工具，这条**编辑取舍**现在写在检查器旁边：**「检查通过了」和「检查看过了」是两句不同的话**。契约层 61 文件 / **1428** 项不变、`node:test` 289 → **290**；读数：产品可执行行黑暗 **32 (0.3%)** 不变 |
| 2026-09-14 | M5（生成出来的状态会过期） | D177：给四个安装检查计时时发现 `plugin:check` 报 **DRIFTED**——第 96 轮从 bundle 删掉 `serveCachedCapabilities` 之后，**已装的那层 operator layer 还带着它**（那一层由 `plugin:install` 从 bundle 推导生成；patch 层的 `config` 是整体替换而不是合并，D74），于是第 96 轮新加的配置检查会在**下一次重启时拒绝装载插件** ✗——**本会话自己的改动造成的、用户可见的破坏**，而它对所有套件都不可见（漂移在 operator 状态里，不在仓库里）。先修环境（`npm run plugin:install`，`plugin:check` 随后报 installed），再让**这个失败自己说清怎么修**：拒绝信息补上一句「多半不是打错字，而是那一层旧了」+ 修复命令 + 重启提示——**一个在重启时才炸的失败必须自带修复命令**，因为它离造成它的那一行太远。两条变异（删掉那句话 / 把命令名写错）都红；`install.md` 补「拉了新代码之后先跑 `plugin:check`」一节。这是「operator 状态」主题连续第三轮现身：不要承诺 store 里有什么（D176）、不要承诺它停在哪个 revision（D176）、以及**从仓库生成出来的东西会在没人看的地方过期**（本条）。契约层 61 文件 / 1427 → **1428** 项；读数：产品可执行行黑暗 **32 (0.3%)** 不变 |
| 2026-09-14 | M5（operator 状态不可承诺） | D176：README 的演示一节写着「**当前项目已推进到 r0023**」并让读者去看 r0019–r0023 的 manifest，而 store 里 `watch-commercial` **只有 r0001 与 r0002**——承诺的 revision 根本不在磁盘上；`render-job.js` 头部同形（「49 of them are already on disk」，实测 4 个）。**根因不是数字过期，而是这一类句子本身**：`.deepblend/projects/` 是 **operator 状态**而不是仓库内容，它现在的样子取决于跑过哪些命令，所以「它现在是什么」是一句**仓库无法保持为真**的话——与 README 不许复述里程碑状态是同一条规则（那条已有检查器），只是对象换成了 store。修法是写**工具会做什么**（那是可检查的）：README 点名 `apply-brief-content.mjs` 与它走的区间（r0018 → r0023），新检查要求演示一节点名工具与区间、而**工具自己的头部注释**里必须真的有这两个 id 且**顺序一致**，并禁止「当前项目已推进到 rXXXX」这种句子回来（**引文也不行**，所以 README 里那句说明只能绕着说）。**第一条检查起初太松**：它搜整个工具源码，于是把区间改成 `r0018 → r0029` 的变异全绿——因为 `r0029` 恰好出现在源码别处的 label 注释里；改成搜头部并按顺序校验后，「改成 r0029」与「区间写反」两条变异都红——**搜得宽，等于没搜**。五条变异全红；契约层 61 文件 / **1427** 项不变、`node:test` 288 → **289**；读数：产品可执行行黑暗 **32 (0.3%)** 不变 |
| 2026-09-14 | M5（复查「到不了」· 第二轮） | D175：把还没复查过的「到不了」说法逐个再量一次，四处里三处有值得改的东西。①`project-store.js` 的索引写入 `catch {}`：又试了**第四条接缝**——把索引文件本身改成只读（`chmod 0444`）——**写居然成功了**（mtime 变了、第二个项目进了索引），因为 `writeFileAtomic` 是**写临时文件再 rename**，而 POSIX 允许 rename 覆盖只读文件（只要目录可写）；「不可测」的结论成立但**理由要写准**，三条被实测否掉的接缝都记进了命名注释。②`json-schema.js` 的 `default: return false`：不可达的原因是编译期拒绝未知类型名（已有断言），本轮补**另一半**——**七个 JSON 类型名逐个正面验证**（各自接受自己的值、拒绝别的类型的值）：**「switch 里今天有七个 case」是关于源码的事实，「这七个值都能被正确判定」才是承诺**；删 case 与让 case 恒真的变异都红。③`render-journal.js` 与 `frame-ledger.js` 两处 `closeSync` 的 `catch` 注释写着 `/* already closed */` ✗——**描述符是该函数自己打开、没有别人关过**，这个情况不可能出现；真正的原因是 **`finally` 里抛出的异常会替换正在传播的错误**，调用者要的是读失败而不是关失败；**一句描述不可能情况的话，会让下一个读者去找一条不存在的路径**。④`png.js` 不可达的 `readPixel` 默认支**抛的是与头部检查一模一样的那句话** ✗——一个文件里同一条拒绝写了两遍；改成它唯一能表达的意思（内部 bug），并更正了 `png-sheet.test.mjs` 里**引用了旧句子**的命名注释——**引文会随被引之物一起烂**。把旧句子改回去的变异**能活过所有行为断言**（分支不可达），于是补了一条**源码级**断言（该拒绝作为抛出的句子只出现一次；注释里引用它可以，那正是守卫解释「为什么不重复它」的方式）——**不可达的分支只能用源码级断言钉住**。五条变异四条红，没红的那条正是源码级断言的来源；契约层 61 文件 / 1425 → **1427** 项；读数：产品可执行行黑暗 **32 (0.3%)** 不变 |
| 2026-09-14 | M1（复查「到不了」的说法） | D174：**更正 D154**。D154（与第 57 轮）写着 `scene-patch.js` 剩下的 6 行黑暗「全部是被 schema 的 `pattern` 遮住的 `PATCH_ID_INVALID`」，并断言「这个文件算跑完了：每一行要么被执行、要么被证明到不了」✗。本轮把十种形状逐个喂进 `validateScenePatch` 复查：九种确实被 schema 先回答，**但 `entity.material.set` 的 `materialId` 在 schema 里没有 `pattern`** ✓——这一形状由**语义层**回答：那 6 行**可达**，而且**一条测试都没有** ✗✗。**一条被解释成「到不了」的黑暗行，是唯一一种会真的藏住缺陷的黑暗行**——而它在两个地方被这样解释了四十轮。修法：写那条测试（`PATCH_ID_INVALID`，path 点名 `operations[0].materialId`、消息带语法），6 行全部变亮，`scene-patch.js` 现在**真的**是 `0/638` ✓；同时收紧旧断言——它写的是 `PATCH_SCHEMA_INVALID || PATCH_ID_INVALID` ✗（**允许两个答案**，于是分支即使变成可达也不会红，又是「两面只测一面」），现在断言**只**是 schema 的码且**不含**语义码。同一次复查确认 `scene-spec.js` 那边的说法**是准的**（七种集合全部由 `SCENE_SCHEMA_INVALID` 先回答）。三条变异全红；读数：产品可执行行黑暗 **38 (0.3%) → 32 (0.3%)**、`scene-patch.js` **6 → 0**、有黑暗行的文件 9 → **8**；契约层 61 文件 / 1424 → **1425** 项 |
| 2026-09-14 | M5（贡献者指南缺了那套做法） | D173：`milestone-status.md` 里「变异」出现 **174 次**，`CONTRIBUTING.md` 里 **0 次**；覆盖探针在贡献者指南里也一次都没提。**一个陌生人读完指南不会知道这个项目怎么决定下一步写什么测试**——而那正是最近六轮一直在用、且每轮都抓到真东西的做法。补两节：**怎么读覆盖探针**（三条命令、看 product CODE lines、**一条黑暗行是一个问题而不是缺陷**（三种答案都要写下来，「测不了」必须带理由）、**探针期间不要改树**、以及**量具的已知盲区**：多行三元的 alternate 会买到一条假黑暗行——先插一句 `process.stderr.write` 确认再决定）与**变异纪律**（**活下来的变异是一个问题**：断言有洞 / 代码到不了 / **你量错了东西**；变异要带上下文瞄准；**两面问题只测一面看不出来**；要证明「我什么都没做」，输入必须带一个默认值不可能等于的值）。同时把 `CONTRIBUTING.md` 加进「命令必须真的能跑」的手册清单——**贡献者指南也是手册**。**随后一条变异没红，指出的是检查器自己的洞**：把指南里一条命令改成 `npm run test:gone  # 必须绿` 没被抓到，因为 `command-claims.mjs` 提取 `npm run <script>` 的模式以 `\s*$` 结尾（**要求整行只有命令**），而本仓库手册习惯在命令后跟注释——于是**任何带注释的命令都不被检查**，指南写一个不存在的脚本可以一路绿。模式放宽为允许行尾有东西（**更宽松的方向是更安全的方向**：点名命令就是声称它存在，误报只是改措辞），并给**检查器自己**加了测试（认得手册真实使用的写法、不从散文里造命令），把模式改回 `\s*$` 的变异让这条测试变红。四条变异全红；契约层 61 文件 / **1424** 项不变、`node:test` 287 → **288**、`run-all.sh` 16 套件全绿；读数：产品可执行行黑暗 **38 (0.3%)** 不变 |
| 2026-09-14 | M1/M5（没人跑的契约） | D172：`deepblend/schemas/job-result.schema.json` 被镜像、被文档、被 SPEC 引用——而**没有任何代码或测试拿它验证过任何东西**。把三个演示项目里 **45 份 attempt log** 喂进去：**45 份全部失败** ✗——`artifacts[]` 的定义不允许 `viewId`/`role`/`at`/`iteration`/`columns`/`rows`/`slot`/`views`，`kind` 里没有 `view` 与 `contact-sheet`，`action` 里没有 `render_views`，而记录一直写着的顶层 `warnings` 数组在 schema 里**根本不存在**。**一份发布出去的契约描述的不是这个产品写的东西，比没有契约更糟——因为读者会信它**；而**没有检查器在跑**，所以谁都没发现。修法：schema 按**实测**补齐（不是猜——`slot` 先被写成整数，实测是视图 id 字符串），补齐后 45/45 通过；`host-render-orchestration.test.mjs` 加三条检查：产品写出的**成功**记录、**失败**记录、以及**带 view + contact-sheet 制品**的记录，都要通过它发布的那个 schema——读的是**磁盘上的原始记录**而不是 `getJob` 的投影（第一版喂了投影进去，报「缺 `schemaVersion`」，正好说明两者不是一个东西）。一条变异起初活着并指出**样本不够**：把 schema 里的 `viewId` 改名，检查全绿，因为被验证的记录里没有 view 制品——**验证一个 schema，要拿它字段最多的那种样本去验**；补上之后三条 view 变异全红。六条变异全红；契约层 61 文件不变、1421 → **1424** 项、`run-all.sh` 16 套件全绿；读数：产品可执行行黑暗 **38 (0.3%)** 不变 |
| 2026-09-14 | M1/M5（悬空引用与照做就错的建议） | D171：把 D169/D170 那把尺子再往下一层——**SceneSpec schema 里每个字段，编译器或契约层读了吗**？三条回来（`license.commercialUse`、`license.attribution`、`asset.license`），顺着 `asset.license` 查下去撞到两件比「没人读」更糟的事：①`scene-patch.schema.json` 把 `asset.add.asset.license` 写成 `{"$ref": "#/$defs/license"}`，而**那个文档里没有这个定义**——于是带许可的资产声明**不是被拒，而是让验证器抛 `SchemaDefinitionError`**：一个调用者无法分支的异常，而契约承诺的是码；②上一轮我刚加的那句建议印的是 `license: "CC-BY-4.0"`（**裸字符串**），而 `asset.license` 是**对象**（`source`/`commercialUse`/`attribution`）——**模型照做会拿到 `SCENE_PATCH_INVALID`，因为它做了被告知要做的事**。两处都修：`license` 定义照抄进 patch schema（验证器只在单文档内解析引用）、建议改成 schema 的形状，并新增 `contract/schema-refs.test.mjs`——**每个 schema 的每个 `$ref` 都必须能解析**（这一类缺陷的通用形式）、带许可的声明被接受、裸字符串拼法被拒、建议句仍用 schema 的形状；最重要的闭环在 `host-asset-ingest.test.mjs`：**答案告诉调用者去写的那条声明要再喂回 `validateScenePatch` 并通过**（**无法照做的建议比没有建议更糟**）。**加这条检查撞上了仓库已有的守卫**（本轮第二次被守卫先找到）：`contract/schema-mirror.test.mjs` 断言打包 schema 与 `deepblend/schemas/` 下的**权威副本**逐字节相同，而我只改了镜像——它立刻报出 357 字节的差异，正是它头部描述的那种缺陷（漂移的镜像＝运行时执行一套与项目信念不同的规则）。权威副本已同样修改并镜像；我新文件里那条「两份 license 定义相同」的检查**删掉了**（逐字节相同是更强的陈述，**一件事一个检查器**）。三条变异全红（悬空引用回来、两份定义漂移、建议改回裸字符串）；契约层 60 → **61** 文件、1412 → **1421** 项、总文件数 75 → **76**、`run-all.sh` 16 套件全绿；读数：产品可执行行黑暗 **38 (0.3%)** 不变 |
| 2026-09-14 | M5（说给模型的承诺） | D170：把 D169 那把尺子移到**面向模型**的一面——每个工具声明的参数，它的 handler 真的读了吗（以及反过来）？解析 `defineTool` 块后读数：**`blender_asset_ingest` 声明了 `license` 而没人读**。描述写着「Licence string recorded with the asset. Worth setting for anything downloaded: an asset whose provenance is unrecorded is one nobody can safely ship.」——而 handler 拼请求时**根本没带上它**，host 那边也**没有 license 字段**，可 SceneSpec 的 asset schema **本来就有 `license`**：这个字符串一直有地方可去，只是没人送。**这比配置键更坏**：读那句描述的是一个**模型**，而模型没有任何办法发现自己在被骗。**修法是兑现承诺而不是删掉承诺**：`ingestAsset` 接受可选 `license`，trim（上限 512 字符）后写进字节旁的 `assets/manifest.json`、随答案返回、并出现在它让模型去写的 `asset.add` 声明里（许可因此一路进 SceneSpec——后来的人真正会看的地方）；没给时记 **`null` 而不是缺字段**（「没人说过」与「没有许可」是两句不同的话）。**检查自己掉进三个坑、每个都被守卫接住**：参数声明有两种拼法（`name: {…}` 与 `name: SHARED_SHAPE`），只认第一种会把 `sceneSpec` 报成「读了却没声明」；「整个 args 交给 helper」的豁免起初匹配到 **`(args, exec)` 函数签名**、把 16 个工具全部豁免——**一个谁都能进的豁免不是豁免而是洞**，于是删掉它、改成「每个工具必须至少声明一个、至少读一个」；按缩进取参数名会把数组参数里的 JSON-Schema 关键字 `items` 当参数，于是改成**按花括号深度取顶层键**。四条变异全红（删掉 `license: args.license`——**就是那个真缺陷**、声明没人读的参数、读没声明的参数、host 不再记许可）；契约层 59 → **60** 文件、1405 → **1412** 项、总文件数 74 → **75**；读数：产品可执行行黑暗 **38 (0.3%) → 38 (0.3%)** |
| 2026-09-14 | M5（配置形状） | D169：SPEC §17 把配置画成**分组**（`finalRender.requireApprovalAboveFrames`、`security.assetMaxBytes`、`jobs.*`、`agent.*`），实现读的是**平铺**的键。实测：Schemastery **接受**分组写法、把它当成不认识的属性留下、**一声不吭**——照 SPEC 抄配置的操作者会得到一个「审批阈值还是默认值」的部署，而没有任何地方报错；**静默失效是配置上唯一不能容忍的失败方式**。新增 `contracts/lib/config-surface.js`：从 schema 的 `toJSON()` 读出这一行声明的键（`dict` 在**根节点**上、根节点是 `refs[uid]`——第一版只读 `json.dict`，于是每个键都被判成不认识的，**被自己的用例当场抓住**），三个 row（host/provider/ui）在构造时检查自己的配置，不认识的键就拒绝并**列出真正读的键**。`install.md` §3.1 是 26 个 SPEC §17 键的完整对照表；偏差表补 **#12**（配置形状）与 **#13**（`requireApprovalAboveResolution` 未实现——帧数是成本的**度量**，分辨率只是其中一个因子，加第二把尺子会造出「哪个先触发」的问题）。**这个检查第一天上班就抓到两个东西**：`serveCachedCapabilities` 声明着而**没人读**（它描述的缓存是 provider 的 `capabilitiesCacheMs` + `getCapabilities({refresh})`，host 里再来一个就是同一个决定的两份拷贝）——删掉它时，它在**十个**测试与工具文件里还留着，每一个都会当场报错，**检查把清理变成了可验证的事而不是 grep 的运气**；以及 `maxMeshPolygons` 被判成没人读，因为它是在同一个包的**另一个文件**里读的，于是检查范围从 `index.js` 改成**整个包**。一条变异起初活了下来并指出真缺口：删掉 host 里那句守卫调用用例全绿——因为用例**直接调用 helper**、从没真的构造过一个 row（**工具被测了，工具的用法没有被测**）；补上「用坏配置真的构造三个服务、断言它抛错」之后，三条同款变异全红。契约层 58 → **59** 文件、1388 → **1405** 项、总文件数 73 → **74**、`run-all.sh` 16 套件全绿；读数：产品可执行行黑暗 **38 (0.3%) → 38 (0.3%)**（产品代码行数 12134 → 12163） |
