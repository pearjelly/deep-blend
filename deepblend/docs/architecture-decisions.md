...
## 5D. 内容补全：把 SPEC §2.1 的需求真正落地（D42–D46）
...
### D43 — SceneSpec v1 只能动**实体 transform**，所以「点亮」必须是几何而不是材质

**触发事实**：`animationTrack.targetEntityId` 必须解析到 **entity**（`scene-spec.js:264`
的 `requiresId('entities', …)`），而 `property` 的枚举只有
`location/rotationEuler/scale` 各分量。于是：

- **相机不能动画** → 「镜头环绕产品」只能表达为产品转台。黑色虚空里没有静止参照物，
  两者视觉上等价。
- **材质不能动画** → 「表盘逐渐点亮」只能是**会缩放的发光几何**：
  一块 `emission` 材质的屏幕盘从 0.001 长到 1，四根 `index-glow` 刻度依次长出。

**这是与 SPEC §2.1 需求之间的真实缺口**，不是实现偷懒：`emissionStrength` 是静态参数，
屏幕只能长出来而不是渐亮。记录在此，留给 schema 演进。

### D44 — 背景不是材质的责任：World 是编译器里写死的常量

**触发事实**：实测 r0018 成片背景是 **(96,96,104)** 的中灰，而 `backdrop-matte` 的
baseColor 已经是 0.022（近黑）。两个原因叠加：

1. 可见的背景根本不是 World 也不是材质色，而是**被照亮的背景墙**——三盏面光很近很强，
   0.022 反照率也能被推到中灰；
2. `deepblend_scene.py:117` 把 World 硬编码成 `(0.02, 0.021, 0.026) × 0.6`，
   **SceneSpec v1 里没有任何 world/background 字段**，规格改不动它。

**决策（在 v1 内）**：`backdrop-matte` / `stage-matte` 置 `baseColor [0,0,0,1]` 且
**`ior = 1`**。后者是必要的：0 反照率的 Principled 表面仍有约 4% 的菲涅耳镜面反射，
那正是灰色残留的来源。实测背景由 (96,96,104) 变为 **(0,0,0)**，产品仍可读。

**遗留缺口**：只要 SceneSpec 不拥有 world 颜色，"黑色背景"就只能靠一块背景板实现；
任何越过这块板的视角仍会看到写死的中灰。
...
### D46 — **未修复的缺陷**：视觉审查只渲一帧，且默认取中点

**触发事实**：r0018 的动画是彻底坏的（D42），而 M2 的真实视觉审查给它 **100 分**。
原因是审查只渲一帧、且默认取区间中点：F45 = 180°，恰好是每个零件都映射到自身的那
一个角度。**中间帧不是"信息量更大"，而是这个缺陷唯一的盲点。**

**状态：记录为缺陷，本次不改。** 修法属于 M2 的视角计划（对动画镜头采多帧而不是一帧），
在 M3 之前应该先决定；否则任何长动画都会带着这个盲点进入渲染。
...
| # | 问题 | 需要在哪个里程碑之前决定 |
|---|---|---|
| Q1 | 帧序列渲染的存储位置 | ✅ **已实测，见下**：450 帧 × 0.94 MiB ≈ **424 MiB**，13 GiB 够用；真正的约束是机时（27.3 s/帧 → 3.4 h/遍） |
...
| 2026-09-13 | 内容补全 | D42–D46：转台＝旋转＋轨道平移、只能动实体 transform、World 是写死的常量、0.75mm 深度间隙、**审查单帧盲点（未修复）** |