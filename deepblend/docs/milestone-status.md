...
### 69.1 第 61 轮：核对「文档里的数字」，并且**没有**改它

这一轮把 README 里那组快照与实际树核对了一遍：`documented-counts`（拥有**文件数/拆分**那一半断言的
检查器）**全绿**，契约文件数 55 ✓、自计文件与 `node:test` 文件的拆分 ✓ 都与 README 一致；
但我自己那个「逐个跑一遍再求和」的脚本这次只数到 **26 个文件 / 1202 项**，而 README 的快照是
**27 个文件 / 1249 项**——差 47 项，恰好是 `store.test.mjs` 的量级（它的摘要形式与其它文件不同）。

**结论按事实写**：快照那一半（总数）没有检查器，本轮**没有**据一次量不准的读数去改它；
下一轮应当先把「哪个文件的摘要没被数进去」找出来，再决定是修脚本还是修数字。
**一个数不出来就先别改的数字，比一个按猜测改过的数字诚实。**
