...
## 203. 首次体验：从零到一张图 166.5 秒，其中 149 秒是下载

### 203.1 轮初读数：没有红

契约层 **71/71** ✓、验收 **17 套件全绿** ✓、九个门禁全部 exit 0 ✓、
CI 连续两次 `success` ✓（`797f68c` ✓、`f9606d0` ✓）。所以这一轮从账本的 ✗ 里选 ✓，
而上一轮 §202.10 的缺口表把它排在第一顺位 ✓。

### 203.2 一条线：一个陌生人会遇到什么

三件事被合成一条线 ✓，因为它们问的是同一个问题 ✓：

* **C6**：从零到「渲出第一帧」要几步、几分钟、卡在哪一步 ✓；
* **C19（本轮新增）**：上游只发布 `linux-x64` 的 Blender ✓——这件事有没有写在用户读到的地方 ✓；
* §202.10 的第二条：`dsh-baseline.md` 没有写明安装命令为什么要多带两个包 ✓。

### 203.3 C6 的读数：一条线两分钟，其中一条腿占了 89%

`verify-clean-clone.mjs` 此前证明「手册那条路走得通」 ✓，**没有记录它要多久** ✓——
而那是 C6 的另一半 ✓。现在每一步都计时 ✓，`--with-blender` 多了一步**用户真正在意的事** ✓：
`create-demo-project.mjs` 在 clone 里跑 ✓、通过真实 provider 渲一张预览 ✓，
然后**从磁盘把那张图读回来** ✓（打印一个路径不等于存在一个文件 ✓）。

| 步骤 | 墙钟 |
|---|---|
| clone | 1.3 s ✓ |
| 初始化 profile（`dsh` 建的，不是安装器 ✓） | 0.1 s ✓ |
| 1. `npm run setup` | 0.2 s ✓ |
| **2. `npm run blender:install`（346 MB 下载 ✓）** | **149.0 s** ✓ |
| 3. `npm run plugin:install` | 0.1 s ✓ |
| 4. `npm run presets:install` | 0.0 s ✓ |
| **5. 第一帧（建项目 ＋ 渲预览）** | **15.8 s** ✓ |
| **合计（用户会遇到的那七步）** | **166.5 s（2 分 47 秒）** ✓ |
| （契约层，不是用户步骤 ✓） | 87.3 s ✓ |
| （完整验收，不是用户步骤 ✓） | 338.6 s ✓ |

**「卡在哪一步」因此是一行字的答案** ✓：那一次 346 MB 的下载 ✓，占用户那条路的 **89%** ✓，
而其余每一步都在零点二秒以内 ✓。它也是**唯一一步用户可以跳过**的 ✓——
自己装 Blender 并设 `blenderPath` ✓，那正是非 macOS 用户做的事 ✓，
也是 `probe-cross-platform.log` 量过的那条路 ✓。

首帧那一步的产物不是一个声明 ✓：**三张 PNG 落在磁盘上** ✓，
最大的一张 186 KB ✓，路径 `.deepblend/projects/watch-commercial/revisions/r0001/previews/camera-main.png` ✓。

**这条读数有一个必须说清的边界** ✓：它是**一台机器、一个时刻、一张这个仓库控制不了速度的网络** ✓
（写在日志头部 ✓，并由 `first-run.test.mjs` 盯着那句话还在 ✓）。
149 秒既是关于产品的事实 ✓，也是关于那条网络的事实 ✓；
**不依赖网络的是形状** ✓——有一步比其它所有步慢两个数量级 ✓，而它恰好是唯一可以跳过的那一步 ✓。

### 203.4 C19：上游的四条发布线，一条 arm64 Linux 都没有

```
$ for v in 5.2 5.1 4.5 4.2; do
    curl -s "https://download.blender.org/release/Blender$v/" | grep -o "blender-$v[0-9.]*-linux[^\"<]*"
  done
Blender 5.2  linux builds: linux-x64.tar.xz
Blender 5.1  linux builds: linux-x64.tar.xz
Blender 4.5  linux builds: linux-x64.tar.xz
Blender 4.2  linux builds: linux-x64.tar.xz
```

**四条线，只有 `linux-x64`** ✓。所以 `install.md` 对非 macOS 用户说的那句
「自己装 Blender 5.2.1」✓ 在 **arm64 Linux 上根本无法执行** ✓，
而此前**没有任何文档说过这件事** ✓。

这一行之所以能在**同一轮里**从 ✗ 变成 ✓ ✓：它问的不是「arm64 Linux 能不能跑」✓
（那是一个上游事实 ✓，产品改变不了 ✓），而是「**这件事有没有写在用户读到的地方**」✓——
后者才是产品能做的部分 ✓。现在三处前置表都写明上游发布的那个产物名 ✓，
其中两处（`README.md` 与 `install.md`）写明 arm64 没有构建 ✓。

### 203.5 `dsh-baseline.md` 补上了那句理由

读者打开那份文档是为了理解**钉住的 harness** ✓，而它对「安装命令为什么多带两个包」一个字都没有 ✓——
上一轮为此花了一整轮 ✓（`Could not locate a DSH deployment` ✓）。
现在它写明：harness 的清单**不带**这两个包 ✓、一次全局安装产出的是**拆开的**依赖树 ✓、
少了它们链接器会以「有包不在部署里」退出 2 并点名 ✓，由 `setup-steps.test.mjs` 盯着 ✓。

### 203.6 变异测试：八条，**一条活了下来**，被收紧的断言杀死
...
| 日志里没有首帧那一步的计时 | 红 ✓ |
| 没有任何帧落盘 | 红 ✓ |
| 下载只用了五秒（即一次没有 `--with-blender` 的重跑） | 红 ✓ |
| `README.md` 不再提上游发布的那个 Linux 构建 | 红 ✓ |
| `README.zh.md` 整段删掉 | 红 ✓ |
| `dsh-baseline.md` 只列包名、不说理由 | 红 ✓ |
| **`install.md` 只提 `linux-x64`、不提 arm64 的缺口** | **活了下来** ✗ |

存活者与上一轮那一条**形状相同** ✓，而且**在同一条规则上失败了两次** ✓：
第一版要求「文件里出现 `arm64`」✓——而 `install.md` 的平台行**本来就有** `macOS arm64` ✓，
所以删掉 Linux 那句之后规则仍然满足 ✓。改成「同一行同时出现」✓——**还是满足** ✓，
因为 `macOS arm64` 就在同一行的前半句 ✓。第三次改成**有方向**的规则 ✓：
先出现上游发布的产物名 ✓、**之后**才出现 arm64 ✓（`/linux-x64[^\n]*arm64/` ✓），
并加了一条反例控制：一个把 arm64 写在 Linux 构建**之前**的句子必须让规则红 ✓。
这一次它红了 ✓。**「同一句话」是有方向的** ✓。

### 203.7 收口读数

* 契约层 **72/72** ✓（本轮新增 `contract/first-run.test.mjs` ✓，四项 ✓）；
  README 快照按 `documented-counts.test.mjs` 报出的真实值改过 ✓
  （71→**72** 个契约文件 ✓、87→**88** 个文件 ✓、88→**89** 条照跑 ✓、
  全新 clone 的读数 `11/71`→**`12/72`** ✓、自计断言 **1581**（34 个文件打印计数）✓——
  最后这个数**没有断言盯着** ✓（它是刻意不设断言的那类快照 ✓），
  所以我按 `count-assertions.mjs` 的读数改 ✓，而不是照我猜的数改 ✓（第一版我写成了 1585 ✓）。
* `bash deepblend/tests/run-all.sh`：**17 套件全绿** ✓。
* 九个包装检查全部 exit 0 ✓，`release:freshness` **FRESH** ✓。
* 本轮**没有改 `packages/**`** ✓，所以四步发布链没有触发 ✓。
...
1. **C12 第三方许可盘点** ✓：仍然没有那份清单 ✓——下一轮的第一顺位 ✓，
   因为它是唯一一行**完全不需要改产品**、却直接决定「能不能商用」的 ✓。
2. **C14 活下来的变异清单** ✓：这一轮与上一轮各有一条存活者 ✓、都被杀死了 ✓，
   但仍然没有一份**常驻**的表 ✓（它最接近可以关闭 ✓）。
3. **C2 / C3**：三条路线的同一性没有断言 ✓；tarball 路线的升级路径未实测 ✓。
4. **C7 运维单点** ✓：只有用户能改变 ✓。
5. **C15 / C16 / C17**：M6 的其余七项 ✓、双语文案 ✓、成本模型 ✓ 都还没有动 ✓。
6. **首次体验的读数只在一台机器上** ✓：它是 macOS arm64 上的 ✓；
   Linux 上那条路的**形状**由 `probe-cross-platform.log` 覆盖 ✓，但**分钟数没有** ✓——
   §202.10 第 4 条说过容器是模拟的 ✓，它的耗时不能当性能读数 ✓。

---

## 204. 第三方许可：一份盘点，和一条「仓库里没有一个别人的字节」的断言

### 204.1 轮初读数：没有红

契约层 **72/72** ✓、验收 **17 套件全绿** ✓、九个门禁全部 exit 0 ✓、CI 连续 `success` ✓。
所以这一轮从账本的 ✗ 里选 ✓，而上一轮 §203.8 把 **C12** 排在第一顺位 ✓——
理由是它是唯一一行**完全不需要改产品**、却直接决定「能不能商用」的 ✓。

### 204.2 量到的现状：一处都没有

账本 C12 的判据栏此前写着「今天的读数：仓库里没有第三方许可清单」✓。逐项去量之后 ✓，
那三件事确实**一处都没有** ✓：① Blender 是外部程序调用 ✓、② ffmpeg 的许可随**用户那份构建**而定 ✓、
③ 受管 Blender 是**下载**而不是再分发 ✓。而这三件恰好是一个公司最先问的三个问题 ✓。

### 204.3 三类关系，一张表，和**读回来的**许可

`deepblend/docs/third-party.md` ✓ 按 `security.md` 的形状写 ✓：每一行指到代码或断言 ✓，
§4 给出复核命令 ✓。它的骨架是三行 ✓：

| 关系 | 例子 | 本产品做了什么 | 义务在谁身上 |
|---|---|---|---|
| **外部程序调用** ✓ | Blender、ffmpeg / ffprobe ✓ | argv 数组启动、不链接、不打包 ✓ | 用户那份安装 ✓ |
| **同行依赖（peer）** ✓ | DSH harness ✓ | `peerDependencies`，由用户**已经装好的**部署提供 ✓ | 用户自己的部署 ✓ |
| **再分发** ✓ | **没有** ✓ | 发布物里只有本仓库自己的文件 ✓ | — ✓ |

许可本身**不是从网站上抄的** ✓，是**读回来的** ✓：

* **Blender 5.2.1 = GPL-3.0-or-later** ✓——读自受管安装**自己带的那份**清单 ✓：
  `head .tools/Blender.app/Contents/Resources/text/license/license.md` ✓
  的第一句就是 *"While Blender itself is released under GPL 3.0 or later"* ✓。
* **本机 ffmpeg 8.1 = GPL-3.0-or-later** ✓——读自 `ffmpeg -version` 的 `configuration:` 行 ✓
  （`--enable-gpl --enable-version3` ✓）。文档同时写明：**许可随用户装的那份二进制而定** ✓，
  别的构建可能是 LGPL ✓，而本产品**不分发任何一份** ✓。

### 204.4 四条断言里最强的一条，不是「文档写了什么」

`deepblend/tests/contract/third-party.test.mjs` ✓ 盯四件事 ✓，而其中两条比文档本身更硬 ✓：

1. **产品 spawn 的外部程序，恰好是配置 schema 声明的三个** ✓——**双向**：从 `packages/**` 里
   推导出 `blenderPath` / `ffmpegPath` / `ffprobePath` ✓，清单里多写一个、少写一个都红 ✓。
2. **仓库里没有一个别人的字节** ✓：`git ls-files` 里没有一个二进制扩展名 ✓
   （`.dmg` / `.exe` / `.so` / `.dylib` / `.node` / `.wasm` / `.tgz` … ✓）；
   而三张被跟踪的图片都在 `docs/images/` ✓、由 `capture-docs-images.mjs` 从本产品拍出来 ✓——
   「我们的图片是我们自己渲的」也是一条断言 ✓。
3. **八个 manifest 的 `license` 与仓库根一致** ✓（今天全部 MIT ✓）。
4. **产品的 `dependencies` 只有自己的包** ✓，其余一律 `peerDependencies` ✓——
   这就是「我们不再分发它」在清单里的写法 ✓。

### 204.5 变异测试：一条**活了下来**，而它是**第三次**同一个形状

| 变异 | 结果 |
|---|---|
| 一个第三方包变成真依赖（`lodash`） | 红 ✓ |
| 一个 manifest 丢掉 `license` | 红 ✓ |
| 仓库里出现一个被跟踪的 `.dmg` | 红 ✓ |
| 清单不再说「再分发会改变结论」 | 红 ✓ |
| 清单不再给出读 ffmpeg 许可的命令 | 红 ✓ |
| 自己的包被写成 peer | 红 ✓ |
| **ffprobe 那一行不再是清单的一行** | **活了下来** ✗ |
| **Blender 那一行不再说「外部程序」** | **活了下来** ✗ |

存活者**第三次**是同一个形状 ✓：**断言接受「文件里出现过这个词」，而不是「那一行说的就是它」** ✓。

1. 第一版：`inventory.includes('ffprobe')` ✓——文档的 §1 散文与 §4 的命令清单里都有它 ✓，所以删掉 §2 那一行仍然通过 ✓。
2. 第二版：改成「某一行同时含 `Blender` 与『外部程序』」✓——**§1 那一行**（讲三类关系的那张表 ✓）
   恰好同时含两者 ✓，所以它又通过了 ✓。
3. 第三版：**把 §2 抽出来** ✓、按**首格是主语**判定 ✓、并按**第三格**判定「怎么被使用」✓——它才红 ✓。

三次的共同点不是正则写错 ✓，是**断言的作用域比它想断言的主张大** ✓。
这条已经连续三轮出现（§202.7 的 ffmpeg 一条 ✓、§203.6 的 arm64 一条 ✓、这一节两条 ✓），
所以下一轮的第一顺位是 **C14：给「活下来的变异」建一份常驻的表** ✓。

**顺带记一条量具自己的错** ✓：这一轮的变异脚本在第一次运行时 **漏删了一个被跟踪的 `.dmg`** ✓，
于是后面三条变异都被**同一个**残留物弄红 ✓，报告成 KILLED ✓。
按手工复跑才发现它们其实**是红的**（这一次是幸运的巧合 ✓），但如果那条变异本来会存活 ✓，
这份记录就会**把一次存活写成一次杀死** ✓。**变异脚本自己也是量具，也会说谎** ✓——
现在它每一条都打印**自己的断言消息** ✓，而不是只打印「KILLED」✓。

### 204.6 收口读数

* 契约层 **73/73** ✓（本轮新增 `contract/third-party.test.mjs` ✓，五项 ✓）；
  README 快照按 `documented-counts.test.mjs` 报出的真实值改过 ✓
  （72→**73** 个契约文件 ✓、88→**89** 个文件 ✓、89→**90** 条照跑 ✓、
  全新 clone 的读数 `12/72`→**`12/73`** ✓）。
  自计断言 **1581**（34 个文件打印计数）✓ 不变 ✓——新文件用的是 `node:test` ✓；
  `node:test` 用例 **402 → 418（38 → 39 个文件）** ✓。
  **这两个数我第一版都猜错了** ✓（写成 1586 / 35 ✓），按工具读数改回来 ✓。
* `bash deepblend/tests/run-all.sh`：**17 套件全绿** ✓。
* 九个包装检查全部 exit 0 ✓，`release:freshness` **FRESH** ✓。
* 本轮**没有改 `packages/**`** ✓（八个 manifest 本来就都声明了 MIT ✓），所以四步发布链没有触发 ✓。

### 204.7 仍然开着的缺口

1. **C14** ✓：连续三轮出现「活下来的变异」✓，形状相同 ✓，但没有一份**常驻**的表 ✓——下一轮第一顺位 ✓。
2. **C17 成本** ✓：token 与渲染时间都还没有成本模型 ✓，而它是唯一一行**用户每次花钱时**都会遇到的 ✓。
3. **C2 / C3** ✓：三条路线的同一性没有断言 ✓；tarball 路线的升级路径未实测 ✓。
4. **C7 运维单点** ✓：只有用户能改变 ✓（第二个账号 ✓）。
5. **C15 / C16** ✓：M6 的其余七项 ✓、双语文案 ✓ 都还没有动 ✓。
6. **许可盘点的两个已知边界** ✓：① 它盘的是**本仓库**的再分发 ✓，
   而**市场**（`awesome-dsh-plugin`）那份列表页有自己的条款 ✓，不在这一行里 ✓；
   ② `third-party.md` 的许可是**读回来的** ✓，但读数会随上游变 ✓——
   它是一份**盘点**，不是一个持续监视 ✓，重跑 §4 的命令才知道今天是什么 ✓。

---

## 205. 活下来的变异：三个形状，和一份只增不减的表

### 205.1 轮初读数：没有红

契约层 **73/73** ✓、验收 **17 套件全绿** ✓、九个门禁全部 exit 0 ✓、CI `success` ✓。
所以这一轮从账本的 ✗ 里选 ✓，而 §204.7 把 **C14** 排在第一顺位 ✓——
理由它自己写着 ✓：同一个形状**连续三轮**出现 ✓，而每一轮都是从头发现的 ✓。

### 205.2 一份表，和它真正要交付的东西

`deepblend/docs/mutation-survivors.md` ✓ 是**追加式**的 ✓，一行一个**洞** ✓：
哪一轮、什么变异、三个形状里的哪一个、**杀死它的断言**、状态 ✓。
但表不是这一行的产出 ✓——**三个形状**才是 ✓：

| 形状 | 它长什么样 | 从哪几条存活者归纳出来 |
|---|---|---|
| **A：作用域比主张宽** ✓ | 断言问「文件里出现过这个词吗」，而主张是「那一行/那一格说的就是它」✓ | §202.7 的 ffmpeg ✓、§203.6 的 arm64 ✓、§204.5 的 ffprobe ✓、§205.5 的本条 ✓ |
| **B：测的是被测对象旁边的东西** ✓ | 断言驱动一个**助手**，而被测的调用者可以不理它 ✓ | §202.7 的链接器计划 ✓ |
| **C：读了一个范围，把它叫作全部** ✓ | 检查读了一个目录、一个 commit 区间、一个 glob，然后当作全体 ✓ | §199.7 的两条 ✓、§205.5 的量具一条 ✓ |

**这三条现在是规则** ✓，写在 §1 里 ✓，而下一份断言写完之后可以拿它去问 ✓：
「我抽出来判定的那一小块，是不是就是我的主张？」✓「我驱动的是入口还是它旁边的助手？」✓
「我读的范围等于我主张的范围吗？」✓ **这是这一行真正的产出** ✓。

### 205.3 表被四条断言盯着，而不是被信任

`contract/mutation-survivors.test.mjs` ✓ 六项 ✓：

1. **每一行都具名一个 killer，而它存在** ✓——指向的文件或命令必须真的在 ✓
   （这一条当场抓到我自己：第一版把 killer 写成 `` `deepblend/tools/release-version.mjs --stale` `` ✓，
   一个**带参数的路径**不是一个可重跑的引用 ✓，改成 `npm run release:freshness` ✓）；
2. **每一行的形状取自闭集** ✓，而闭集由**断言**持有 ✓、与文档 §1 声明的集合**相等** ✓
   （D38 的形状 ✓：一份词汇表，或者两份都不算数 ✓）；
3. **每一行引用的 `milestone-status.md` 小节真的存在** ✓——引文要落得到地方 ✓
   （写这一节之前它就已经红了 ✓，因为 §205.5 还不存在 ✓）；
4. **文档的散文里不许出现计数** ✓——行数就是计数 ✓。
   第一版这里写过一句「哪几轮报告了零存活」✓，而且**同一句话里就数错了** ✓——
   一份自己数不清自己的表比没有表更糟 ✓，所以那一句被删掉 ✓，并加了一条断言不许它回来 ✓。

### 205.4 账本的数字由表推导，不是抄的

账本的 §2 多了一行「活下来的变异（累计）」 ✓，而它的**读法**是「数 `mutation-survivors.md` §2 的行」 ✓。
所以表里加一行而账本没跟着动 ✓，账本会红 ✓（这一轮就是这样发现它该从 6 变成 7 的 ✓）。
这正是账本对**所有**数字的规矩 ✓，而这一条是它第一次用在一个**跨文档**的读数上 ✓。

### 205.5 变异测试：七条，而那条「存活」是**量具在说谎**

| 变异 | 结果 |
|---|---|
| 一个 killer 指向不存在的文件 | 红 ✓ |
| 一行用了闭集外的形状（`D`） | 红 ✓ |
| 一行引用不存在的 `milestone-status.md` 小节 | 红 ✓ |
| 文档散文里出现一个计数 | 红 ✓ |
| 表里删掉一行存活者 | 红 ✓ |
| 表里加一行而账本没跟着动 | 红 ✓（账本那条断言 ✓） |
| **§1 多出第四个形状的行** | **第一次报告 `SURVIVED`** ✗ → 见下 |

那条「存活」是**两次**发现的合体 ✓，而两次都是**量具**的问题 ✓，不是断言的洞——**但第一次里藏着一个真的洞** ✓：

1. **真的洞** ✓：`mutation-survivors.test.mjs` 第一版的正则只认 `[ABC]` ✓——
   **它只能看见自己已经知道的那些形状** ✓，所以一个 `D` 行对它**不可见** ✓，两个集合永远相等 ✓。
   这是形状 **A** 的第四次出现 ✓，也是第一次**量具自己**的作用域比主张窄 ✓。修法：取 §1 表里的**任意字母**再比较 ✓。
2. **量具的错** ✓：我重跑这条变异时，替换模式没有匹配上文档里那个**加粗的单元格** ✓，
   文件**一个字节都没变** ✓，而套件当然全绿 ✓，于是报告成 `SURVIVED` ✓。
   **一条没有应用的变异，与一条活下来的变异长得一模一样** ✓。
   修法写进 §3 的第二条 ✓：每一条变异**先断言文件真的变了** ✓，变了才去跑套件 ✓，
   否则报告 `NOT-APPLIED` ✓。

**两轮之内，量具说谎两次** ✓（§204.5 的残留 `.dmg` ✓、这一条 ✓），而两次都会把「洞还开着」记成「洞关上了」✓。
所以 §3 现在是一张**独立的表** ✓，与 §2 同价 ✓。

### 205.6 收口读数

* 契约层 **74/74** ✓（本轮新增 `contract/mutation-survivors.test.mjs` ✓，六项 ✓）；
  README 快照按 `documented-counts.test.mjs` 报出的真实值改过 ✓
  （73→**74** 个契约文件 ✓、89→**90** 个文件 ✓、90→**91** 条照跑 ✓、
  全新 clone 的读数 `12/73`→**`13/74`** ✓、`node:test` 用例 **418 → 424（39 → 40 个文件）** ✓，
  自计断言 **1581**（34 个文件打印计数）✓ 不变 ✓）。
* `bash deepblend/tests/run-all.sh`：**17 套件全绿** ✓。
* 九个包装检查全部 exit 0 ✓，`release:freshness` **FRESH** ✓。
* 本轮**没有改 `packages/**`** ✓，所以四步发布链没有触发 ✓。
...
1. **C17 成本** ✓：token 侧与渲染时间侧都还没有成本模型 ✓，而它是唯一一行**用户每次花钱时**都会遇到的 ✓——
   下一轮第一顺位 ✓。它需要的读数很具体：一次视觉审查花多少 token ✓（`visual-review-live-probe.mjs` 是现成的入口 ✓）、
   一次渲染按参考机的秒数换算成分钟 ✓（工具描述里已有 19.6–41.4 s/帧 ✓）。
2. **C2 / C3** ✓：三条路线的同一性没有断言 ✓；tarball 路线的升级路径未实测 ✓。
3. **C7 运维单点** ✓：只有用户能改变 ✓。
4. **C15 / C16** ✓：M6 的其余七项 ✓、双语文案 ✓ 都还没有动 ✓。
5. **§1 的三条规则还没有被用在「写之前」** ✓：它们现在是**事后**的判决 ✓
   （一条变异活下来 → 归到某个形状 ✓）。真正省时间的是**写之前**拿它问一遍 ✓——
   这件事今天没有任何机制 ✓，只有这一节散文 ✓。

---

## 206. 成本：量它的过程挖出一条更大的缺陷——视觉审查整天没有工作

### 206.1 轮初读数：没有红

契约层 **74/74** ✓、验收 **17 套件全绿** ✓、九个门禁 exit 0 ✓、CI `success` ✓。
所以这一轮从账本的 ✗ 里选 ✓，而 §205.7 把 **C17（成本）** 排在第一顺位 ✓。

### 206.2 量 token 的第一步就撞上了一条红

C17 说 token 侧「一个读数都没有」✓，而现成的入口是 `visual-review-live-probe.mjs` ✓
（一次真实的模型调用 ✓）。跑它 ✓：

```
resolved route: {"provider":"deepseek-official","model":"deepseek-flash",…,"inputModalities":["text"]}
finish: {"kind":"error","failure":{"message":"The model 'deepseek-flash' does not exist (distributor)","code":"HTTP_404"}}
usage: null
```

**两个事实** ✓：产品配的那个视觉模型**这个部署已经不提供了** ✓；
而它解析出来的路由写着 `inputModalities: ["text"]` ✓——**就算它存在，它也看不见图** ✓。
目录里实际有的那个 ✓ 是 `deepseek-v4.1-flash` ✓，带 `["text","image"]` ✓。

### 206.3 更糟的是：它已经这样一整天了，而每个套件都是绿的

`.deepblend` 里**全部十份**视觉审查记录 ✓ 都记着同一句话 ✓：

> the vision reviewer could not be consulted, so this review carries measurements and a sheet but
> no second opinion: The vision reviewer's model call failed: The model 'deepseek-flash' does not exist

**M2 的招牌能力（视觉闭环）整天没有工作** ✓。循环**按设计**优雅降级 ✓——
它记下失败 ✓、继续走测量 ✓、把「没有第二意见」写进记录 ✓——
然后**没有人读那条记录** ✓。这是这一轮最贵的一课 ✓：
**一个优雅降级的能力，会安静地退化成一个不再工作的能力** ✓，
而「没有报错」正是它安静的原因 ✓。

### 206.4 修法：默认值、**花钱之前**的检查、以及一个点名的码

1. **默认值**改成实测存在且接受图像的模型 ✓（`deepseek-v4.1-flash` ✓），
   bundle patch 里那段「M2 探针实测 `deepseek-flash` 接受图像」的注释**当时是真的** ✓，
   现在改成了这一次的读数 ✓。
2. **在花钱之前检查路由** ✓：`_visionRoute()` ✓ 查三件事——provider 在不在 ✓、
   模型在不在 ✓、`inputModalities` 含不含 `image` ✓。前两件失败会得到 404 之外的东西 ✓；
   **第三件才是安静的那一半** ✓：一个纯文本模型会**愉快地回答** ✓，从不看那张图 ✓，
   于是审查带着零 findings 回来 ✓——**与「看了，没问题」下游完全一样** ✓。
   检查是**尽力而为**的 ✓：harness 没有目录（老版本 ✓）时跳过 ✓，模型调用仍然是权威 ✓。
3. **新码 `VISUAL_REVIEW_MODEL_UNAVAILABLE`** ✓，消息里列出**目录里哪些模型可以** ✓，
   并写进 `recovery.md` §10 ✓。加这个码的时候，`error-documentation.test.mjs` 当场红了 ✓：
   「put it in EXPLAINED with the section that tells a reader what to do, or in NOT_EXPLAINED under
   a group that says why a reader never needs it」✓——**一个码要有一个决定** ✓，这是那条断言的用处 ✓。

**修完之后的读数是同一条命令** ✓：路由解析成带 `["text","image"]` 的模型 ✓、
`finish: {"kind":"stop"}` ✓、**模型真的描述了画面** ✓
（"A grey square camera-like device with a black oval face, centered in the frame." ✓）。

### 206.5 探针自己也在说谎，两处

两处都是**同一个毛病**：探针带着**产品配置的第二份副本** ✓。

1. **它自带模型名** ✓（`MODEL = … ?? 'deepseek-flash'` ✓）——所以产品修好之后 ✓，
   它还在问那个死名字 ✓，报告出与修之前**一模一样**的 404 ✓。改成读
   `StudioConfig({}).visualReviewModel` ✓。
2. **它自带 token 预算** ✓（`maxTokens: 400` ✓）——于是模型把 400 全花在推理上 ✓、**一个字都没吐** ✓
   （`finish: max-tokens` ✓），而那正是产品用 `visualReviewMaxTokens` 防的事 ✓，
   被探针的第二份副本原样复现 ✓。改成读产品的预算 ✓。

**一条探针如果自带被测对象的配置，它量的是自己** ✓。

### 206.6 C17 的两半

**渲染侧** ✓：那个每帧秒数（19.6–41.4 s ✓，1920×1080 / Cycles / 256 samples ✓）
此前是**两处副本** ✓（审批提示里的字符串 ✓ + 工具描述里的句子 ✓），现在是**一处定义** ✓
（`REFERENCE_SECONDS_PER_FRAME` ✓），而审批提示把乘法做完了 ✓：

```
450 帧 → about 2.5 hours to 5.2 hours of machine time
```

由 `describeRenderCost()` 从**帧数**算出来 ✓，所以它随请求变化 ✓、未知帧数答「unknown」而不是猜 ✓、
单数复数也对 ✓（第一版写成「1 minutes」✓，用户会读到那句话 ✓）。

**token 侧** ✓：两次**真实调用** ✓——

| 图 | 输入 | 输出 | 其中推理 | 合计 |
|---|---|---|---|---|
| 单视角 640×360（6 KB）✓ | 345 ✓ | 704 ✓ | 686 ✓ | **1049** ✓ |
| 真实 2×2 contact sheet（1014×630，37 KB）✓ | 548 ✓ | 192 ✓ | 178 ✓ | **740** ✓ |

**读法本身就是结论** ✓：图大了**六倍** ✓，总 token 差不多 ✓——
**成本由推理决定** ✓，而推理量随问题而变 ✓，所以给区间 ✓、不给单个数 ✓。

### 206.7 变异测试：七条，七条全红，没有一条「没应用」
...
| 花钱之前不检查路由 | 红 ✓ |
| 纯文本模型被当成审查器 | 红 ✓ |
| 文档里的 token 数与探针读数不一致 | 红 ✓ |

这一轮的变异脚本**每一条都先断言文件真的变了** ✓（§205 那条教训 ✓），
所以「NOT-APPLIED」这一档没有被用到 ✓——而它在脚本里 ✓，因为**一条没有应用的变异与一条活下来的变异长得一模一样** ✓。

### 206.8 收口读数

* 契约层 **75/75** ✓（本轮新增 `contract/cost-model.test.mjs` ✓，五项 ✓；
  `visual-reviewer` 23→**32** 项 ✓）；README 快照按真实值改过 ✓
  （74→**75** 个契约文件 ✓、90→**91** 个文件 ✓、91→**92** 条照跑 ✓、
  自计断言 1581→**1590**（34 个文件打印计数）✓、`node:test` 424→**429**（40→**41** 个文件）✓、
  全新 clone 的读数 `13/74`→**`13/75`** ✓）。
* `bash deepblend/tests/run-all.sh`：**17 套件全绿** ✓。
* 九个包装检查全部 exit 0 ✓。
...
1. **C2** ✓：三条路线各自都验过 ✓，**只有「它们相同」没有断言** ✓——
   它是 §199.9 以来最老的一个具名缺口 ✓，下一轮第一顺位 ✓。
2. **C3** ✓：tarball 路线的升级路径未实测 ✓。
3. **C7 / C15 / C16** ✓：运维单点 ✓、M6 的其余七项 ✓、双语文案 ✓。
4. **一次**成功**的视觉审查不记 `usage`** ✓：只有空回答那条失败把它记在错误详情里 ✓，
   所以「一轮真实循环花多少 token」今天仍然量不到 ✓——`cost.md` §3 把这一格写明了 ✓。
   这是**下一轮可以补的一格** ✓，也是这一行唯一还缺的读数 ✓。
5. **一次会话的累计 token** ✓：产品任何地方都不聚合 ✓。
6. **默认模型的正确性只能对着活目录验** ✓：契约层测不了「这个部署今天服务哪个模型」 ✓，
   所以那一条的判据是**探针**（它现在读产品自己的路由 ✓）。

---

## 207. 三条路线服务同一个产品：第一次有了断言，而它当场被证明能红

### 207.1 轮初读数：没有红

契约层 **75/75** ✓、验收 **17 套件全绿** ✓、九个门禁 exit 0 ✓、CI `success` ✓。
所以这一轮从账本的 ✗ 里选 ✓，而 §206.10 把 **C2** 排在第一顺位 ✓——
它是 **§199.9 以来最老的一个具名缺口** ✓，连着八轮的缺口表都点过它 ✓。

### 207.2 缺口的形状：三条都验过，而「它们相同」没有

三条路线各自都有读数 ✓（`probe-dsh-plugin-{npm,github,tarball}.log` ✓），
三条用的是**同一套判据** ✓——而**没有任何东西断言它们一致** ✓。
「一条命令跑三遍」是**习惯**，不是**检查** ✓。而它们**可以**不一致 ✓：
三条从三个不同的地方取东西 ✓（默认分支 ✓、registry ✓、Release 资产 ✓），
而不一致的方式是**安静的** ✓——**每一条都成功地报告了一个不同的产品** ✓。

### 207.3 产出：一条命令，四条判据，其中一条**必须不同**

`npm run release:parity` ✓ 装三次 ✓、把三份读数**互相比较** ✓：

| 判据 | 为什么是它 |
|---|---|
| 三条的 `installed version` **互相相同** ✓ **且**等于 `deepblend/version.json` ✓ | 仓库自己那句话是**第三个意见** ✓——两条**都陈旧**的路线互相一致 ✓，是这个检查否则会叫作健康的状态 ✓ |
| 三条的 `/deepblend/workbench` 都成功回答 ✓ | 版本号是产物**对自己的声明** ✓；这条路由是**只有当前版本才有**的 ✓，陈旧产物在那里只能回答 404 ✓ |
| preset 相同 ✓ | 三条都承诺交付两个 preset ✓ |
| **`packages pnpm fetched` 必须不同：7 / 7 / 1** ✓ | **这一条是这一行的关键** ✓：只找「一致」的检查 ✓，会在三条**什么都没量到**时同样通过 ✓；tarball 把兄弟包装在产物里 ✓，那个数就是「三条真的是三条路」的证据 ✓ |

**pnpm 的 store 是空的** ✓：tarball 的 URL 按市场规则**不带版本号** ✓，
所以它跨版本**逐字节相同** ✓，而按 URL 取键的 store 会把上一个产物给你 ✓（§199.6 量到过 ✓）。

### 207.4 它当场被证明能红——而那是它一半的价值

一个**从没被看见报过不一致**的比较 ✓，是一个**没人测过**的比较 ✓。所以工具带一条负控 ✓：

```
$ DEEPBLEND_PARITY_NPM_SPEC='@deepblend/dsh-blender-bundle@0.2.1' npm run release:parity
npm installed: @deepblend/dsh-blender-bundle@0.2.1
PROBLEM: the three routes serve 2 different versions: …@0.2.2 | …@0.2.1
problems: 2
result: 2 reading(s) say the three routes do not agree
[exit=1]
```

那条命令写在**三处** ✓：工具头部 ✓、日志头部 ✓、`CONTRIBUTING.md` §5 ✓——
**一个没人知道怎么触发的检查，等于一个没人测过的检查** ✓。

### 207.5 抽出来：比较是一个函数，不是脚本里的一段

第一版把比较写在 `release-route-parity.mjs` 里 ✓，于是契约层够不到它 ✓——
而**量它的代价是三次安装** ✓（每次几分钟 ✓）。实测撞到这一点 ✓：
`import` 那个脚本为了拿到函数 ✓，会把整条探针**跑一遍** ✓。
所以比较搬进 `tools/route-parity.mjs` ✓（探针 import 它 ✓，测试 import 它 ✓），
于是七条变异**在毫秒级**跑完 ✓，而不是每条几分钟 ✓。
**这是「不要复制，要抽出来」的另一个方向** ✓：抽出来不只是为了不重复 ✓，是为了**能被驱动** ✓。

### 207.6 变异测试：七条，七条全红，没有一条「没应用」
...
| 不再检查那个**必须不同**的数 | 红 ✓ |
| 不同的 preset 不再被报告 | 红 ✓ |
| tarball 路线被期望取与另外两条一样多的包 | 红 ✓ |
| 已提交的读数被改成自相矛盾 | 红 ✓ |

脚本每一条都先断言文件真的变了 ✓（§205 的教训 ✓），所以 `NOT-APPLIED` 一档没有被用到 ✓。

### 207.7 收口读数

* 契约层 **76/76** ✓（本轮新增 `contract/route-parity.test.mjs` ✓，九项 ✓）；
  README 快照按真实值改过 ✓（75→**76** 个契约文件 ✓、91→**92** 个文件 ✓、92→**93** 条照跑 ✓、
  `node:test` 429→**438**（41→**42** 个文件）✓、自计断言 **1590** 不变 ✓、
  全新 clone 的读数 `13/75`→**`14/76`** ✓）。
* `bash deepblend/tests/run-all.sh`：**17 套件全绿** ✓。
* 九个包装检查全部 exit 0 ✓，`release:freshness` **FRESH** ✓。
* 本轮**没有改 `packages/**`** ✓（只加了工具、测试、文档与一条 npm script ✓），所以四步发布链没有触发 ✓。

### 207.8 仍然开着的缺口

1. **C3** ✓：tarball 路线的**升级**路径未实测 ✓——它与这一轮是同一个家族 ✓：
   三条都装得上 ✓、都服务同一个版本 ✓，而**「装过旧版的那台机器能不能升上来」**仍然没有读数 ✓。
   它正是 §199.6 那个陷阱的下一步 ✓（同一个 URL 会给你旧产物 ✓），下一轮第一顺位 ✓。
2. **C7 / C15 / C16** ✓：运维单点 ✓、M6 的其余七项 ✓、双语文案 ✓。
3. **一次成功的视觉审查仍然不记 `usage`** ✓（§206.10 第 4 条 ✓）：`cost.md` §3 写明了这一格 ✓，
   补它需要让**成功**那条路也把 `usage` 写进审查记录 ✓——那是一行代码加一条断言 ✓，下一轮可做 ✓。
4. **一次会话的累计 token** ✓ 仍然没有聚合 ✓。
5. **默认模型的正确性只能对着活目录验** ✓：这一轮的 `release:parity` 不覆盖它 ✓
   （它验的是三条路线一致 ✓，不是模型在不在 ✓）——判据仍然是那个探针 ✓。

---

## 208. 升级路径：两条路线的行为不一样，而机制只有一个

### 208.1 轮初读数：没有红

契约层 **76/76** ✓、验收 **17 套件全绿** ✓、九个门禁 exit 0 ✓、CI `success` ✓。
所以这一轮从账本的 ✗ 里选 ✓，而 §207.8 把 **C3** 排在第一顺位 ✓——
它是安装那一族的最后一格 ✓。

### 208.2 缺口：用户问的**第二个**问题没有答案

三条路线都装得上 ✓、都服务同一个版本 ✓（C2 ✓）。而用户问的**第二个**问题是 ✓：
**「我装的是上个月的构建，怎么拿到这个月的？」** ✓ `install.md` 里**没有答案** ✓，
因为没有人把这个问题当成一条命令跑过 ✓。

值得一条探针而不是一段说明的理由 ✓：tarball 路线的资产名**不带版本号** ✓
（市场的 `/releases/latest/download/` 规则 ✓），所以 URL 跨版本**逐字节相同** ✓，
而 pnpm 的 store 正是按那个 URL 取键的 ✓——一台装过上一个 release 的机器再问同一个 URL ✓，
拿到的是上一个产物 ✓（§199.6 量到过 ✓，而它看起来**与一次失败的发布一模一样** ✓）。

### 208.3 怎么让它可测：按 **tag** 取旧资产

两个旧 spec 都是真的、而且今天仍然可达 ✓：
registry 上的 `@deepblend/dsh-blender-bundle@0.2.1` ✓，
以及 `…/releases/download/v0.2.1/deepblend-bundle.tgz` ✓——
**Release 资产按 tag 是可达的** ✓，哪怕那个不带版本号的名字只会服务最新的那份 ✓。
这条事实是这个问题**今天还能被回答**的原因 ✓（再往后，旧 tag 的资产会被删掉 ✓）。

每条路线各拿一个临时 `$DSH_HOME` 与一个**空的** pnpm store ✓，
所以没有缓存能替网络回答 ✓。

### 208.4 答案：两条路线不一样，而机制只有一个

```
npm      0.2.1 → 重新 add 裸包名   → 还是 0.2.1（记录下来的 spec 没变：0.2.1）
                → remove + add      → 0.2.2（spec 变成 ^0.2.2）
tarball  0.2.1 → 重新 add 当前 URL → 0.2.2（spec 变了：v0.2.1 → latest）
```

**机制** ✓（这一轮真正量到的东西 ✓）：**生态的 `add` 不会动一个已经存在的依赖的 spec** ✓。
npm 路线上那个 spec 还是旧钉子 ✓，所以它不动 ✓；
tarball 路线上 `v0.2.1` → `latest` **就是换了一个 spec** ✓，所以它动 ✓。

**两条都有效的命令**是 `dsh plugin remove` 再 `dsh plugin add` ✓——
现在写进 `install.md` §6 的升级一节 ✓，**连同理由** ✓：
只给两条命令而不说「为什么只跑第二条不够」 ✓，正是这个仓库一直在替换掉的那种说明 ✓
（读者会先试显而易见的那个 ✓、看着它**安静地什么都没做** ✓、然后断定升级坏了 ✓——
而那正是 npm 路线的行为 ✓）。

**读回来的那条 spec 行是这份日志比一张版本表多出来的东西** ✓：
版本说**发生了什么** ✓，spec 说**为什么** ✓。

### 208.5 变异测试：六条，一条活了下来，而它**存活了两次**

| 变异 | 结果 |
|---|---|
| 手册把两条升级命令的顺序写反 | 红 ✓（顺序就是那条指令 ✓） |
| 手册把那句理由删掉 | 红 ✓ |
| 源码路线的推断被写成「也是实测的」 | 红 ✓ |
| 手册不再引用它来自的那份读数 | 红 ✓ |
| 读数里 npm 路线永远到不了当前版本 | 红 ✓ |
| **读数里那条说明机制的 spec 行被删掉** | **活了下来** ✗ |

存活者的形状是 **A** ✓（作用域比主张宽 ✓），而它**存活了两次** ✓：
第一版要求「至少四条 spec 行」✓——**任何四条都满足** ✓；
第二版要求「某条 npm 行的值等于旧版本」✓——而重新 `add` 之后那一行**也是** `0.2.1` ✓，
所以删掉**最初**那条仍然通过 ✓。第三次改成按**精确标签**要求那一对 ✓
（`npm recorded spec:` 与 `npm recorded spec now: ^<当前>` ✓），它才红 ✓。
**「某一条」与「那一条」的差别，在这一轮里值两次收紧** ✓。

### 208.6 量具第三次说谎：三条「存活」其实没有应用

这一轮的临时变异循环里 ✓，**三条**报成 `SURVIVED` 的其实**根本没有应用** ✓：
一条是 shell 把模式里的**反引号当成命令替换**吃掉了 ✓，
两条是模式与文件里的文本不匹配 ✓（我把中文标点记错了一处 ✓）。
**§3 第一条的规矩（先断言文件真的变了 ✓）没有写进那些临时循环** ✓——
规矩在 ✓、脚本在 ✓，而**临时循环每次都是新写的** ✓，于是它每次都可以忘 ✓。
这一条已记进 `mutation-survivors.md` §3 的第三行 ✓，规矩也写清了 ✓：
**任何一次变异都在同一个脚本里跑，而那个脚本对每一条都先断言文件变了** ✓。

### 208.7 收口读数

* 契约层 **77/77** ✓（本轮新增 `contract/upgrade-path.test.mjs` ✓，四项 ✓）；
  README 快照按真实值改过 ✓（76→**77** 个契约文件 ✓、92→**93** 个文件 ✓、93→**94** 条照跑 ✓、
  `node:test` 438→**442**（42→**43** 个文件）✓、自计断言 **1590** 不变 ✓、
  全新 clone 的读数 `14/76`→**`15/77`** ✓）。
* `bash deepblend/tests/run-all.sh`：**17 套件全绿** ✓。
* 九个包装检查全部 exit 0 ✓，`release:freshness` **FRESH** ✓。
* 本轮**没有改 `packages/**`** ✓，所以四步发布链没有触发 ✓。
...
1. **C16 双语文案** ✓：剩下三行里**用户感知最强**的一行 ✓——一个非中文用户装完之后
   看到的是一整屏中文 ✓，而它今天连「要不要做」都还没有决定 ✓。下一轮第一顺位 ✓。
2. **C7 运维单点** ✓：只有用户能改变 ✓（第二个账号 ✓，或把发布权交给仓库所属的账号 ✓）。
3. **C15** ✓：M6 的其余七项 ✓，每一项都是一个功能轮 ✓（第一项是 Blender Live Bridge ✓）。
4. **旧 Release 资产会消失** ✓：这一轮的升级读数依赖「按 tag 取 v0.2.1 的资产」 ✓，
   而资产是可以被删掉的 ✓。下一次重跑这份探针时，如果那个 tag 的资产没了 ✓，
   它会在第一步就红 ✓（`installing … produced nothing` ✓）——那是**正确的**失败 ✓，
   但它意味着这份读数有一个**会过期的前提** ✓，写在日志头部 ✓。
5. **源码路线的升级是推断** ✓：它跟默认分支 ✓，没有旧版本可起点 ✓，
   所以它那一条按机制推出来 ✓、并**在手册里标明是推断** ✓，由断言盯着那个标注 ✓。

---

## 209. 产品文案的语言：平台的契约已经回答了「要不要双语」

### 209.1 轮初读数：没有红

契约层 **77/77** ✓、验收 **17 套件全绿** ✓、九个门禁 exit 0 ✓、CI `success` ✓。
所以这一轮从账本的 ✗ 里选 ✓，而 §208.8 把 **C16** 排在第一顺位 ✓——
它的判据栏里写着「**连「要不要做」都还没有决定**」 ✓，这一轮要做的第一件事就是**把它变成能回答的问题** ✓。

### 209.2 先量，再决定：harness 自己怎么做的

量下来的三件事 ✓，每一件都改变了这个问题的性质 ✓：

1. **harness 自带一个 locale 包** ✓：`dsh-client-locale` ✓，支持两个 locale（`zh` / `en` ✓）。
2. **它的回退规则是一句话** ✓：`FALLBACK_LOCALE = "en"` ✓，注释原话是——
   *"English is both the locale the UI opens in when the browser names no registered language (and for
   non-browser runs), and the dictionary consulted after the active locale misses a key ... because a
   browser naming no registered language is **the reader least likely to read Chinese**"* ✓。
3. **当前 locale 写在页面上** ✓：`document.documentElement.lang = snapshot.active === "zh" ? "zh-CN" : snapshot.active` ✓。

而 DeepBlend 的工作台此前在**每一处**用户可见位置硬编码中文 ✓——
于是**平台特意路由到英文的那个读者，看到的是一整屏中文** ✓。
**这正是那条规则存在的理由** ✓。所以「要不要双语」不是偏好问题 ✓：
**产品在违反它所运行的那个平台的契约** ✓。

### 209.3 答案：跟随部署的 locale，回退英文

**138 条文案**收进一个带 `#region strings` 标记的表 ✓（`zh` / `en` 两侧**键集相同** ✓——
那是 harness 自己的不变量 ✓），locale 从页面读 ✓，回退英文 ✓，`t(key, params)` 支持占位符 ✓。

**表的位置不是随便放的** ✓：它必须在 §B（词汇表）**之前** ✓——
第一版放在 §C 之前 ✓，而 §B 的六个 tab 标签就在那里用 `t()` ✓，
于是加载即 `ReferenceError: Cannot access 't' before initialization` ✓。
**抓到它的是 `ui-cards.test.mjs`** ✓（那个套件真的把 bundle 加载起来渲染 ✓）——
**这就是「组件被断言存在」与「组件被真的调用过」的差别** ✓。

### 209.4 一条断文案的断言，会在文案被翻译时红

迁移之后 `workbench-page.test.mjs` 红了两条 ✓，而它断的是**中文原文** ✓
（`/已提交 r0004/` ✓、六个 tab 的 label 与契约逐字相同 ✓）。
这个套件没有 `document` ✓，按平台规则回退英文 ✓，所以它现在看到的是英文 ✓。

**修法不是把期望值改成英文** ✗——那只是把同一根钉子换个方向钉 ✗。
改成**断事实** ✓：标签断 id 与「非空」 ✓、提交消息断它点名的 **revision** ✓。
**一条断文案的断言，会在文案被翻译时红；一条断事实的断言不会** ✓。

### 209.5 变异测试：七条全红，三条值得点名
...
**第三条「表外又出现一句中文」是让这张表有意义的那一条** ✓：
一张装着文案、而渲染路径各自留着自己的字面量的表 ✓，会通过上面所有检查 ✓，
而用户看到的**一个字都不会变** ✓。**双向**那条同理 ✓：
用了不存在的键会渲染出键本身 ✓（用户看到一个洞 ✓），而表里没人用的键是**腐烂的开始** ✓。

### 209.6 收口读数

* 契约层 **78/78** ✓（本轮新增 `contract/workbench-copy.test.mjs` ✓，五项 ✓；
  `workbench-page` 49 → **51** 项 ✓）；README 快照按真实值改过 ✓
  （77→**78** 个契约文件 ✓、93→**94** 个文件 ✓、94→**95** 条照跑 ✓、
  `node:test` 442→**447**（43→**44** 个文件）✓、自计断言 **1590** 不变 ✓、
  全新 clone 的读数 `15/77`→**`16/78`** ✓）。
* 表：两侧各 **136** 键 ✓（第一版 138 ✓，其中两个是我先写进表、后来发现没有调用点的 ✓——
  被**双向**那条断言当场抓到并删掉 ✓）。
* `bash deepblend/tests/run-all.sh`：**17 套件全绿** ✓。
* 九个包装检查全部 exit 0 ✓。
* 本轮**改了 `packages/**`** ✓，所以四步发布链触发 ✓（见 §209.7 ✓）。

### 209.7 M4 浏览器套件：十条断言断的是**文案**，而它们同时依赖跑测机器的语言

迁移之后 M4 红了 **10 条** ✓，而它们红的原因有两层 ✓，第二层比第一层重要得多 ✓：

1. 表层 ✓：它们断的是中文原文 ✓（`/已创建/` ✓、`/已提交 r0002/` ✓、`/进程实测已消失/` ✓……）。
2. **里层** ✓：这个套件的读数**取决于跑它的那台机器的浏览器语言** ✓——
   产品跟随 locale 之后 ✓，同一条断言在中文机器上绿、在英文机器上红 ✓。
   **它此前之所以一直绿，是因为产品一直说中文** ✓——一个被硬编码掩盖掉的依赖 ✓。

第一版修法是**把浏览器语言钉成 `zh-CN`** ✓（`--lang=zh-CN` ✓）。**实测：不生效** ✓——
harness 解析出的 locale 没有变 ✓。所以钉语言这条路要么更重（去写部署的 `locale` 设置 ✓）、要么不可靠 ✓。

**改成让套件不依赖语言** ✓：十条断言全部改成断**事实** ✓——
新建项目断它点名的 **id** ✓、提交断 **r0002** ✓、渲染断它服务的 **revision** ✓、
时间断**时分秒的形状** ✓、取消断 **job id** ✓、诊断断 **404** 与**两种语言的「非 JSON」** ✓。
**这才是对的方向** ✓：断文案的断言会在翻译时红 ✓，断事实的不会 ✓——
...
1. **C15** ✓：M6 的其余七项 ✓——账本上唯一**装着功能**的一行 ✓，第一项是 Blender Live Bridge ✓。
   下一轮第一顺位 ✓。
2. **C7 运维单点** ✓：只有用户能改变 ✓（第二个账号 ✓）。它已经在缺口表里连续两轮 ✓。
3. **服务端文案没有盘点** ✓：这一轮迁的是**客户端** ✓（用户看到的 ✓）。
   而**模型读到**的文案（工具结果、错误消息 ✓）是另一个平面 ✓，
   它今天仍然是中英混用 ✓——例如 `blender_job_status` 的 `已取消 …，进程实测已消失` ✓
   与 `ENCODER_NOT_FOUND` 的英文消息 ✓ 同时存在 ✓。
   这是**新量出来的一个缺口** ✓：模型把中文结果转述给英文用户 ✓，
   于是「跟随平台 locale」在那一侧**还没有做** ✓。
4. **切换语言不会实时生效** ✓：这一轮的 locale 在模块加载时解析一次 ✓，
   而 harness 的 locale 切换会让整棵树重渲染 ✓（它的 renderer 按 revision 缓存 `t` ✓）。
   工作台要跟着切，需要订阅那个 revision ✓——**今天没有** ✓，写在 `client.js` 的注释里 ✓。

---

## 210. 一个 Blender 进程，很多次操作：Live Bridge 的传输半边

### 210.1 轮初读数：没有红

契约层 **78/78** ✓、验收 **17 套件全绿** ✓、九个门禁 exit 0 ✓、CI `success` ✓。
所以这一轮从账本的 ✗ 里选 ✓，而 §209.9 把 **C15** 排在第一顺位 ✓——
它是账本上唯一**装着功能**的一行 ✓，而 `SPEC.md` §20 的 `M6` 列表把下一项写成 **Blender Live Bridge** ✓。

### 210.2 先量清楚「那一项」有多大

`SPEC.md` §135 写着 *"MVP 使用 Blender Batch。Blender GUI Live Bridge 放到后续阶段。"* ✓——
而 §20 的 `M6` 列表里 **Live Bridge** 与 **Blender Add-on** 是**两项** ✓。
量下来它们是**一件事的两半** ✓：传输（一个 Blender 进程连着服务很多次请求 ✓）
与附着（用户自己那个 GUI Blender + add-on ✓，让他能看着它干活 ✓）。

**传输半边可以在一轮里做完并断言** ✓；附着半边要 GUI 与 add-on ✓，是下一轮的事 ✓。
所以这一轮做传输 ✓，并且**在记录与账本里都写明它不是一整项** ✓——
**一次只开一条线**不等于把一条线说成已经走完 ✓。

### 210.3 读数：每次操作从约 865 ms 降到约 40 ms

同一台机器 ✓，同样三次 `get_capabilities` ✓：

| | 耗时 | 进程数 |
|---|---|---|
| **批处理**（今天所有其他路径） | **2594 ms** | **3** ✓ |
| **会话**（`openSession()`） | **714 ms 开一次 + 119 ms** | **1** ✓ |

**每次操作约 865 ms → 约 40 ms** ✓，**第二个操作就是回本点** ✓。
机制不神秘 ✓：批处理每次都要起一个 Blender、`--factory-startup`、加载场景 ✓；
会话把这些付**一次** ✓。

**这条读数不是引用的** ✓：`live-session.e2e.mjs` 的第五项**每次都重新量** ✓——
「三次操作在一个会话里比三次批处理便宜」 ✓。一次量出来的读数，是**那一次**的读数 ✓。

### 210.4 协议只有一份

会话**复用** `bootstrap.py` 自己的派发 ✓：同一张 `ACTIONS` 表 ✓、同一套协议版本检查 ✓、
同一个 `build_envelope` ✓、同一份进程身份写入 ✓。
**第二份实现就是第二个答案** ✓——「`compile_scene` 到底做什么」这个问题只允许有一个回答 ✓。
会话模式下新增的只有**对话本身** ✓：一行一个请求 ✓、一行一个信封 ✓、`shutdown` 与 EOF 收尾 ✓。

失败按**请求**隔离 ✓：未知动作以**批处理那条码**回来 ✓（`BLENDER_UNSUPPORTED_ACTION` ✓），
而**会话继续可用** ✓——「这次请求错了」与「这个进程坏了」是两件事 ✓。

### 210.5 变异测试：四条全红，其中一条**曾经活下来**

| 变异 | 结果 |
|---|---|
| 去掉「进程死了」的监听 | **曾经活下来** ✗ → 补断言后红 ✓ |
| 忽略 `shutdown` 请求 | 红 ✓（补了「说过再见」那条之后 ✓） |
| 答案不套 `build_envelope` | 红 ✓ |
| 未知动作不再用批处理那条码 | 红 ✓ |

**活下来的那条是本轮最有价值的产出** ✓：原来的断言只走到「关掉之后再问被拒绝」 ✓，
而那条路是**客户端自己的 `closed` 标志**给的 ✓，与**进程真的死了**无关 ✓。
于是补三条 ✓：从外面 `kill -9` 之后 ✓，请求必须**以码回答**而不是等满超时 ✓、
必须**很快**回答 ✓、而且消息要说**进程没了**而不是怪这次请求 ✓。改完它就红了 ✓。

**同一条**变异也暴露了「说过再见」这一格 ✓：`shutdown` 被忽略时 ✓，
`close()` 仍然靠**关掉 stdin**让循环自然结束 ✓——所以进程照样消失 ✓、断言照样绿 ✓。
**一个不能失败的实现细节** ✓，除非有断言看着它 ✓：现在 `bye` 被记下来并被断言 ✓。

### 210.6 量具第五次说谎：脚本自己把工作树弄脏了

这一轮的临时变异脚本犯了**两个**错 ✓：
① `apply` 与 `case_run` 分成两步 ✓，于是「没应用」被报成 `SURVIVED` ✓（**第四次**同一个形状 ✓）；
② 脚本里两个变量**重名** ✓（备份目录与 bootstrap 路径都叫 `B` ✓），
于是 `restore` **把文件复制到了错误的路径** ✓，**工作树里留下了变异** ✓。

第二个错差点造成真正的损失 ✓：我用 `git checkout --` 去清 ✓，
而那条命令把**这一轮尚未提交的实现**一起还原掉了 ✗✗——
正是这份记录在 §199 附近记过的那个错 ✓（**又一次** ✓）。**救回来的**是变异脚本的备份目录 ✓
（它在实现之后、变异之前拍的 ✓）。三件事因此写进规矩 ✓：
① 应用与运行是**一个函数** ✓；② `restore` **用改动前的 sha256 校验自己** ✓；
③ **在跑任何变异之前先提交** ✓——这一轮之后它是硬规矩 ✓。

### 210.7 一条我自己埋的红，也在这一轮修掉

`plugin-install-path.test.mjs` 里那条断言**去读了 registry** ✓——而契约层的承诺是**不需要网络** ✓。
**实测：写它的那一轮通过 ✓，下一轮就红 ✓**，原因与本仓库无关 ✓。
改成只断**读取器的离线契约** ✓（三种答案 ✓、够不到 registry 时是 `null` **而不是** `false` ✓——
「查不了」不是「没发布」 ✓），活的读数留在 `release:parity` ✓。

### 210.8 收口读数

* 契约层 **78/78** ✓；验收 **18 套件**（本轮新增 `blender-integration/live-session.e2e.mjs` ✓，15 项 ✓）。
* 账本多了一行 ✓（**C20** ✓），而 `C15` 仍然开着 ✓、并写明了**它差的是哪一半** ✓。
* 本轮**改了 `packages/**`** ✓，所以四步发布链触发 ✓（见 §210.9 ✓）。

### 210.9 发布链：读回来那一步，这一轮被它自己咬了一口

```
0. version 0.2.3 -> 0.2.4 ✓ + version:sync ✓
1. release:tarball ✓（385.1 kB ✓）
2. gh release create v0.2.4 ✓
3. publish:packages -> **第一个包就报「npm 说成功，而 registry 不服务它」** ✗
```

**那是我上一轮加的读回来那一步** ✓，而它**是对的** ✓——它读到的确实是「registry 还没有这个版本」 ✓。
**但它把「还没有」当成了「没有」** ✗：`contracts@0.2.4` 明明发布成功了 ✓，
只是还没传播 ✓——**本仓库已经把那个窗口记过三次** ✓（packument 约两分钟 ✓、tarball 约五分钟 ✓），
而**我这一轮又走了一次** ✓（第三次 ✓）。

修法两处 ✓，而第二处是**第一次修出来的新毛病** ✓：

1. **读回来要等** ✓：`waitForRegistry()` 有界等待 ✓（240 s ✓），
   而**够不到 registry** 时仍然答 `null` 而不是 `false` ✓（「查不了」不是「没发布」 ✓）。
2. **等的位置错了** ✗：第一版**每个包**都等满窗口 ✓——
   于是两分钟的延迟被乘了七 ✓，一次真实发布跑过十分钟**被我的命令超时杀掉** ✗，
   留下**五个已发、两个没发** ✓。**传播是 registry 的性质，不是某个包的性质** ✓：
   逐包的读是**快读** ✓，有界等待只发生**一次** ✓——在最后那句「七个都在 registry 上」里 ✓，
   因为那句话才是主张 ✓。
3. 顺带修一句**言过其实的话** ✓：那条失败消息写着「after the propagation window」 ✓，
   而它其实**一秒都没等** ✓。**一句夸大自己勤勉的话，是读者学会不信的那一种** ✓。

**收尾读数** ✓：七个包全部在 registry 上 ✓（**逐个读回来** ✓），
`release:parity` 三条路线一致 ✓、`problems: 0` ✓，两份探针日志按 0.2.4 重写 ✓。

### 210.10 仍然开着的缺口

1. **C15 的下一半** ✓：**GUI 附着** ✓——M6 列表里的 **Blender Add-on** ✓，
   它与这一轮的传输半边合起来才是 **Live Bridge** 那一项 ✓。下一轮第一顺位 ✓。
2. **C7 运维单点** ✓：只有用户能改变 ✓（第二个账号 ✓）。**已经在缺口表里连续三轮** ✓——
   按本目标的规矩 ✓，**同一个需要人的条件连续三轮不变**才可以报 blocked ✓；
   它是目前唯一一个这样的条件 ✓。
3. **会话还没有被任何工具用上** ✓：`openSession()` 今天只有测试在调 ✓。
   把它接进工具面（让 `blender_scene_patch` 之类在同一个进程里连着跑 ✓）是**产品面的下一步** ✓，
   而它需要一次工具面的设计决定 ✓（哪些工具走会话 ✓、会话什么时候关 ✓）✓。
4. **账本引用的小节曾经没有被断言盯着** ✓——**本轮已关闭** ✓（见 §210.11 ✓）。

### 210.11 账本自己的引文：一条曾经没有守卫的规则

这一轮写 C20 的判据时引用了 `milestone-status.md` §210.3 ✓，而**那一节当时还不存在** ✓——
契约层却**全绿** ✓。`mutation-survivors.test.mjs` 从它写出来那天起就检查
**存活者表**引用的每一节真的存在 ✓，而**账本自己**没有这一条 ✓。
**一条引文落不到地方，读起来像证据** ✓——失败的样子不是「数字错了」 ✓，是「读者照着找，什么也没找到」 ✓。

补的断言只认**点明了文件名的引文** ✓（`` `milestone-status.md` §N `` ✓）：
第一版匹配了账本里**所有** `§N` ✓，于是它报出 `§0` ✗——
那是账本引的 `install.md` §0 ✓，属于**另一份文档** ✓，去里程碑记录里找它是**一次冤枉** ✓。
**一条会冤枉人的断言，比没有断言更糟** ✓，因为它教会人忽略它 ✓。

---

## 211. Blender Add-on：让产品在你正看着的那个 Blender 里干活

### 211.1 轮初读数：三条红，而它们都是**我自己**造成的

契约层 **78/78** ✓、`run-all` 报了**三条红** ✗：
`documented-counts` ✗、`live-session` ✗、`Host composition activation` ✗。
逐条查完 ✓，**三条都不是产品缺陷** ✓：

1. **`documented-counts`** ✗：我这一轮新建的 `deepblend_bridge.py` 让「动作模块」的计数从 6 变成 7 ✓——
   而**它不是动作模块** ✓（它实现 0 个动作 ✓）。**真缺陷在规则里** ✓：那条规则假定「除了 dispatcher 与工具之外都是动作模块」 ✓。
   修法是**具名例外** ✓，不是把 README 的数字改成 7 ✓——**一个打开 `deepblend_bridge.py` 找动作的读者什么也找不到** ✓。
2. **`live-session`** ✗：那条基线**跑在我编辑那个文件的同一分钟** ✓。
   **这不是读数** ✓——本仓库早就有这条规矩 ✓（**跑动中被改动的树，它的读数不算读数** ✓），
   而这一轮我**又**踩了一次 ✓：**基线在后台跑，而我在前台改同一个文件** ✓。
3. **`Host composition activation`** ✗：「挂载 bundle 不启动任何 Blender 进程」 ✓——
   而我的冒烟测试留下了一个 Blender ✓。**探针看见了它** ✓，那条断言是对的 ✓。

**这一轮的第一课是流程的** ✓：**基线必须读完再动手** ✓。
我把 `run-all` 丢进后台 ✓、没读它的结果就开始写代码 ✓，
于是三条红在我发现之前一直是「未读」状态 ✓——而其中一条（计数）**确实是本轮的题** ✓。

### 211.2 那一项的两半：Add-on 与会话合起来才是 Live Bridge

`SPEC.md` §20 的 `M6` 列表里 **Blender Live Bridge** 与 **Blender Add-on** 是两项 ✓，
而它们是一件事的两半 ✓：**会话**（一个进程连着服务很多次请求 ✓，§210 ✓）
与**附着**（那个进程是**用户自己那个 Blender** ✓，让他看着它干活 ✓）。
这一轮做后半 ✓，于是**两项一起完成** ✓。
...
* **作为 Blender 插件** ✓（GUI 那一半 ✓）：用户装上、勾上 ✓，他自己的 Blender 就开一个本地 socket ✓，
  并在 `View3D` 侧栏加一个 **DeepBlend** 面板 ✓，写着它监听在哪 ✓、服务过多少次 ✓、最后一次是什么 ✓。
* **作为无头服务** ✓（测试能驱动的那一半 ✓）：
  `blender --background --python deepblend_bridge.py -- --socket <path>` ✓，**同一套协议** ✓、没有 GUI ✓。

**两个入口而不是两个文件** ✓：协议与派发是**一件事** ✓——为无头那一半再写一份 ✓，
就是「`compile_scene` 做什么」的第二个答案 ✓，而它会**先烂** ✓，因为没有东西跑它 ✓。
它 import 的还是 `bootstrap.py` 自己那张 `ACTIONS` 表 ✓、同一个协议版本检查 ✓、同一个信封 ✓。

### 211.4 传输是一个缝，不是一个分支

产品这一侧新增 `attachSession(socketPath)` ✓——而它**不是** `openSession()` 的第二个实现 ✓：
两者的**对话**完全一样 ✓（握手 ✓、每次请求的截止时间 ✓、把答案配回请求 ✓），
只有**谁拥有另一端**不同 ✓。所以对话被抽成 `BlenderSession` ✓，
而下面挂着**两个适配器** ✓：`SubprocessTransport`（我们 spawn 的进程 ✓）与
`SocketTransport`（用户那个 Blender ✓）。

**为什么这个缝值得存在** ✓：写两遍意味着下一个 deadline 的 bug 有两个地方要修 ✓，
而**第二个地方是没人跑的那个** ✓。

### 211.5 一次挂起，教出协议里缺的一行

`attachSession()` 第一次跑就**挂住** ✓：`session.ready()` 永远不返回 ✓。
原因：无头入口在**启动时**往 stdout 打了一行 `{"kind": "ready"}` ✓，
而一条**连接**什么都没有 ✓——**产品在等一个已经在服务的对端开口** ✓。
修法是在每条连接开头补上同一行握手 ✓：**同一个对话，就该用同一种方式开口** ✓。

### 211.6 用户真正会遇到的那件事：Blender 把插件**单独**复制走

**Blender 会把插件文件复制到它自己的 add-ons 目录** ✓——
于是 `deepblend_bridge.py` 落地时是**孤零零一个文件** ✓，它 import 的 `bootstrap.py` **不在旁边** ✓。
无头那一半**永远看不到这个** ✓（那里文件还在包里 ✓），**而每一个真的装它的人都会遇到** ✓。

所以搜索顺序是三处 ✓：我旁边 ✓ → 操作者指的环境变量 `DEEPBLEND_BOOTSTRAP_DIR` ✓ →
用户在**插件偏好设置**里填的目录 ✓。三处都没有时 ✓，**失败点名要设什么** ✓，
而不是把一个 `ImportError` 抛进一个什么都不显示的面板 ✓。

**这一条是被断言的** ✓：套件把一个**孤立的副本**喂给 Blender ✓，
断它输出的是「bootstrap.py was not found」与那个变量名 ✓——**用户的那条路，不能只有人记得才被走过** ✓。

### 211.7 变异测试与读数

| | |
|---|---|
| `live-session.e2e.mjs` | **22 项** ✓（上一轮 15 ✓：新增附着的六项 ✓ 与孤立副本那一项 ✓） |
| 附着之后 `get_capabilities` | `success` ✓，**两次同一个 pid** ✓（那是**被附着那个 Blender 自己的 pid** ✓，不是宿主的 ✓） |
| 未知动作 | `BLENDER_UNSUPPORTED_ACTION` ✓，**连接继续可用** ✓ |
| 关闭附着 | 用户的 Blender **继续活着** ✓（它是他的 ✓） |
| 没有人监听时附着 | 有码拒绝 ✓，消息点名**开插件**与那个脚本 ✓ |

### 211.8 变异测试：一条**活下来的**（假存活）与一条**只能靠挂住来失败**的

| 变异 | 结果 |
|---|---|
| 连接不再发握手 | 红 ✓ |
| 插件不再找 `bootstrap.py`（直接 `import`） | 红 ✓ |
| 握手里的 pid 不再被记下来 | 红 ✓（`null` ✓） |
| 构造时把 pid 设成**宿主自己**的 | **活了下来** ✗ → 见下 |
| 把附着的握手期限改成一天 | **跑不完** ✗ → 见下 |

**假存活** ✓：那条变异是**空操作** ✓——握手那一行**会覆盖**构造时设的值 ✓，
所以把构造函数里的 pid 改成宿主的 ✓，运行起来还是被附着的那个 Blender 的 ✓。
**我改的是代码里一个被覆盖的赋值** ✓，不是行为 ✓。
换成「握手里的 pid 不再被记下来」之后 ✓，它当场红 ✓（`null` ✓）。

**只能靠挂住来失败** ✓：把期限改成一天之后 ✓，套件**不是红** ✓，是**不动** ✓——
而「不动」与「我这台机器上的命令超时了」**长得一模一样** ✓。
**一条只能靠挂住来失败的变异，是一个没法读的读数** ✓：它测的是我的耐心，不是产品 ✓。

### 211.9 而这一轮的变异跑出了一个**真的洞**

第一次跑「连接不再发握手」时 ✓，套件**挂住了** ✓——不是红 ✓，是不动 ✓。
查下去发现的是**产品里的洞** ✓：`BlenderSession.ready()` **没有期限** ✓。
一个**连上了但什么都不说**的对端 ✓（socket 开着 ✓、永不开口 ✓）会让调用者**永远等下去** ✓。
**挂起比失败更糟** ✓：它没有消息 ✓、没有码 ✓、也没有结束 ✓，
而本仓库的规矩是「失败必须是可分支的结果」 ✓。

修法两处 ✓：`ready()` 有了期限 ✓（同一个理由：**对话的开场与一次请求一样要有边界** ✓），
而**附着**用的是更短的期限 ✓（10 s ✓）——这不是偏好 ✓，是差别 ✓：
spawn 出来的 Blender **合理地**要一两秒才启动 ✓，而一个**已经在服务**的插件立刻就能回答 ✓。
**实测**：用 provider 自己那个 180 s 的期限 ✓，一个连上不说话的 peer 要 180 s 才报出来 ✓——
**长到用户会以为产品挂了** ✓。

### 211.10 收口读数

* 契约层 **78/78** ✓；验收 **18 套件** ✓（`live-session` 从 15 → **22** 项 ✓）。
* 账本的 **M6 完成项 1 → 3** ✓（Live Bridge ✓ 与 Add-on ✓ 一起完成 ✓）——
  这是这一轮的**可数量化读数改善** ✓。
* `install.md` 多了一节 ✓：怎么装这个插件 ✓、**为什么还要告诉它 `bootstrap.py` 在哪** ✓、装完看到什么 ✓。
* 本轮**改了 `packages/**`** ✓，所以四步发布链触发 ✓（见 §211.10 ✓）。

### 211.11 仍然开着的缺口

1. **工具面还没有用上会话或附着** ✓：`openSession()` / `attachSession()` 今天只有测试在调 ✓。
   把它们接进工具（哪些工具走会话 ✓、会话什么时候关 ✓）是**产品面的下一步** ✓，
   而它需要一次设计决定 ✓。**这是 C15 之外最该动的一件事** ✓。
2. **C7 运维单点** ✓：只有用户能改变 ✓（第二个账号 ✓）。**连续四轮** ✓。
3. **GUI 面板本身没有被自动化测过** ✓：这一轮断言的是**插件模块** ✓（注册 ✓、服务 ✓、孤立副本的失败 ✓），
   而**面板画出来长什么样** ✓ 需要一块真的屏幕 ✓——今天只有人打开 Blender 才能看见 ✓。
   写在 `install.md` 里的是**怎么装** ✓，不是「它一定好看」 ✓。
4. **默认 socket 路径是全局的** ✓（`~/.deepblend-bridge.sock` ✓）：同一台机器上两个工作区 ✓
...
## 212. 会话接进工具面：把「批处理不是唯一形态」这句话兑现

### 212.1 轮初读数：没有红（而且这一次我**读了**它）

契约层 **78/78** ✓、验收 **96 条 ✓**、`ALL SUITES PASSED` ✓。
上一轮的教训是**流程的** ✓：我把基线丢进后台、没读它就开始写代码 ✓，
于是三条红在我发现之前一直是「未读」 ✓。**这一轮先读完再动手** ✓。

### 212.2 缺口：能力有了，而没人用

§211.11 第 1 条 ✓：`openSession()` / `attachSession()` 是**运行时能力** ✓，
而**只有测试在调** ✓——「批处理往返不是唯一形态」这句话**还没有兑现** ✓。

### 212.3 先量那个能让整件事作废的问题：复用进程会不会串味

一个进程连着编译两个场景 ✓，第二个会不会看见第一个的残留 ✓？
`reset_scene()` 每次编译开头都调 `bpy.ops.wm.read_factory_settings(use_empty=True)` ✓
（**整份 `bpy.data` 重置** ✓）——**看起来**没问题 ✓，而这个仓库的规矩是**量它** ✓。

实测（`leak5` ✓，两份**真的不同**的 spec ✓，差**一个实体** ✓）：
...
| A（会话） | 9 ✓ |
| B（批处理） | **10** ✓ |
| B（会话，紧接在 A 之后） | **10** ✓ ← 不是 19 ✓ |
| A 再来一次（在 B 之后） | 9 ✓ |

**没有串味 ✓，也没有累积 ✓**，而**这个测试看得见差别** ✓（9 ≠ 10 ✓）——
**一个看不出差别的泄漏测试，什么也没测** ✓。

### 212.4 一次失败教出会话缺的东西：请求自带的参数

第一次尝试复用进程时 ✓，**每一个真动作都失败** ✓：
`compile_scene requires --scene-spec` ✓。
原因：`compile_scene` 要 `--scene-spec` ✓、`render_preview` 要 `--output` ✓，
而批处理是把它们当**额外 argv** 传的 ✓——**会话的 argv 在进程启动时就定死了** ✓。

修法不是「带新参数重启进程」 ✗（那就是批处理 ✓），
而是**让请求自带参数** ✓，并由**同一个解析器**解析 ✓：
**一个解析器** ✓，于是批处理里能用的 flag 在这里也能用 ✓，
**包括不认识的 flag 要同样失败** ✓。

### 212.5 设计决定：哪些动作走会话，哪些不走

`sessionActions` 默认 **`['get_capabilities', 'compile_scene']`** ✓，而**这是一次决定，不是一个默认值** ✓：

* 这两件事**纯而短** ✓，而且**问了一次通常马上又问一次** ✓
  （模型改一版就编译一次、读一次、再改再编译 ✓）。读数：§210.3 量到每次操作约 865 ms → 约 40 ms ✓。
* **渲染刻意不在里面** ✓：取消一次渲染必须**恰好杀掉那一次渲染** ✓，
  而这个 provider 唯一能做到的方式是**结束进程** ✓——**在一个被附着的 Blender 上，那就是用户自己的会话** ✓。
  **为了省一秒而复用一个进程，不值得拿「不能停一次渲染而不关掉别人的 Blender」去换** ✓。

会话**不是缓存** ✓：它被**闲置计时器**关掉 ✓（默认 60 s ✓），
也**随这一行一起 dispose** ✓（`ctx.effect` ✓）——一个被卸载的插件不该留下一个 Blender ✓。
而计时器**不会在请求进行中关会话** ✓：第一版会 ✓，
于是一次**还在跑的编译**被以「closed by its caller」拒绝 ✓——
**一次什么都没做错的请求，被闲置逻辑结束了** ✓。**闲置就是闲置** ✓。

**而它是 OPT-IN 的** ✓——**这是这一轮最重要的一条** ✓。
第一版把这两个动作**默认**路由进会话 ✓，于是**三条验收套件红了** ✓，**而它们是对的** ✓：
**保活的会话有一份更窄的契约** ✓。

| 批处理承诺的 | 会话给不了 |
|---|---|
| 每次调用**自己的 stdout 捕获**（有上限 ✓、有洪水测试 ✓） | 会话的 stdout 是**对话本身** ✓，没有可以读的尾部 ✓ |
| 每次调用的**截止时间会杀掉进程** ✓ | 会话的期限**不杀进程** ✓（M5 的套件在进程表里找到了它 ✓） |
| **每次调用的目录用完即清** ✓ | 会话**持有**它的目录 ✓，只要它活着 ✓（M0 的探针找到了残留 ✓） |

**这些都不是会话的 bug** ✓，是**另一种形状、另一套承诺** ✓。
而**一个悄悄把一套承诺换成另一套的默认值** ✓，正是这个仓库的套件存在的理由 ✓。
所以快的那条路**要显式打开** ✓：操作者自己点名哪些动作 ✓，
而安全的那些就是**纯而短**的那两个 ✓（`get_capabilities` ✓、`compile_scene` ✓）。

### 212.6 读数：同一个动作，走运行时

| | 耗时 | 进程 |
|---|---|---|
| 三次 `get_capabilities`（批处理，§210.3 的读数） | 2594 ms ✓ | 3 ✓ |
| 三次 `get_capabilities`（**走运行时** ✓） | **832 ms** ✓ | **1** ✓ |

而 `closeSession()` 之后 ✓，**那个 pid 由操作系统确认已经消失** ✓（进程数回到 1 ✓）。

### 212.7 变异测试：四条，一条**活了下来**
...
| 会话动作退回一个进程一个 | 红 ✓ |
| 渲染也被路由进会话 | 红 ✓ |
| **关会话时只丢引用、不关进程** | **活了下来** ✗ → 补断言后红 ✓ |

**存活者又是那个形状** ✓（形状 **B** ✓）：我那条断言查的是 `_session === null` ✓，
而**「关掉了」与「忘了」都满足它** ✓——**一个标志对进程什么也没说** ✓，
正是这个文件里**死亡路径**早先教过的那一课 ✓。
改成问**操作系统** ✓（那个 pid 还在不在 ✓），它当场红 ✓。

### 212.8 收口读数

* 契约层 **78/78** ✓；`live-session.e2e.mjs` 24 → **32 项** ✓。
* 本轮**改了 `packages/**`** ✓，所以四步发布链触发 ✓（见 §212.10 ✓）。

### 212.9 仍然开着的缺口

1. **C7 运维单点** ✓：只有用户能改变 ✓（第二个账号 ✓）。**连续五轮** ✓。
2. **GUI 面板本身没有被自动化测过** ✓（§211.11 第 3 条 ✓）：这一轮没有动它 ✓。
3. **默认 socket 路径是全局的** ✓（§211.11 第 4 条 ✓）：同一台机器上两个工作区会撞 ✓，没有断言 ✓。
4. **附着还没有被工具面用上** ✓：`attachSession()` 是运行时能力 ✓，
   而**工具面没有「用哪个 Blender」的选择** ✓——今天走会话的是我们自己 spawn 的那个 ✓。
   让用户能说「就用我开着的这个」 ✓ 是下一步 ✓，而它需要一次配置面的决定 ✓。
...
## 213. 「就用我开着的这个 Blender」：从一条运行时能力变成一次配置

### 213.1 轮初读数：没有红（读完才动手）

契约层 **78/78** ✓、验收 **96 条 ✓**、`ALL SUITES PASSED` ✓。

### 213.2 缺口：能力在，而没有人能说那句话

§212.9 第 4 条 ✓：`attachSession()` 是**运行时能力** ✓，而**工具面没有「用哪个 Blender」的选择** ✓——
今天走会话的永远是**我们自己 spawn 的那个** ✓。也就是说 ✓：
「在你正看着的那个 Blender 里干活」这件事 ✓ **只有测试做得到** ✓。

### 213.3 修法：一个键，而不是一个新工具

`sessionSocket` ✓：设了它 ✓，`sessionActions` 里点名的动作就由**那个 socket 后面的 Blender** 服务 ✓。
**没有加第 17 个工具** ✓——「用哪个 Blender」是**部署的性质** ✓，不是一次调用的参数 ✓；
把它做成工具 ✓ 会让模型在每一次调用里都要重新选一遍一个它不该关心的事 ✓。
...
**两个键都要** ✓，因为快的路是**显式打开**的 ✓（§212.5 的教训 ✓：
保活的会话**承诺得更少** ✓）。

### 213.4 设了而没人应答：报错，**不**悄悄改成自己起一个

这是这一轮**唯一有争议的决定** ✓，而它站在仓库这一边 ✓：
**用户要的是他自己那个 Blender** ✓。**悄悄起一个别的、然后报成功** ✓，
会让用户看到操作成功 ✓、而永远不知道**动手的不是他那个窗口** ✓——
**「做到了别的事还报成功」正是这个仓库反复付代价的那种失败** ✓。
所以拒绝**有码** ✓、**点名要开哪个插件** ✓，而这一条**有断言** ✓。

### 213.5 读数与断言

`live-session.e2e.mjs` 32 → **36 项** ✓，新增的四条是这一轮的全部主张 ✓：

| 断言 | 它挡住的是什么 |
|---|---|
| 配置了 socket 之后 ✓，动作由**那个 pid** 服务 ✓ | 悄悄 spawn 一个 ✓（变异实测：红 ✓） |
| 关掉产品的连接之后 ✓，**用户的 Blender 还活着** ✓ | 把「关连接」做成「杀进程」 ✓（变异实测：红 ✓） |
| 配置了没人应答的 socket ✓，**有码拒绝并点名插件** ✓ | 静默回退 ✓ |
| 用户那个 Blender 在运行时被指向**之前**就已经在服务 ✓ | 断言在一个空的 socket 上通过 ✓ |

### 213.6 变异测试：两条红，一条**是空操作**

| 变异 | 结果 |
|---|---|
| 忽略配置的 socket，照样 spawn | 红 ✓ |
| `close()` 里顺手把 pid 杀掉 | 红 ✓ |
| 在 `SocketTransport.settle()` 里杀 `this.pid` | **活了下来** ✗ → **它是空操作** ✓ |

**第三条是本轮关于量具的那一课** ✓：`SocketTransport.pid` 在构造时是 `null` ✓——
真正的 pid 是**握手**送来的 ✓，记在**会话**上 ✓，不在传输上 ✓。
所以那条变异**什么都没改** ✓，而「什么都没改」与「活下来了」长得一模一样 ✓（**第七次** ✓）。
换成第二条（在 `close()` 里杀**会话的** pid ✓，也就是有人真的会写的那种「清理」 ✓）之后 ✓，它当场红 ✓。
**一条变异要先证明自己改了东西** ✓——这条规矩在 `mutation-survivors.md` §3 里 ✓，
而它每一条**临时循环**都还是要重新想一遍 ✓。

### 213.7 收口读数

* 契约层 **78/78** ✓；`live-session.e2e.mjs` **36 项** ✓。
* `install.md` 的插件一节改掉了那句已经过时的话 ✓（「工具面还没有自动改用它」 ✓），
  换成**两个键**与**为什么设了 socket 却没人应答要报错** ✓。
* 本轮**改了 `packages/**`** ✓，所以四步发布链触发 ✓（见 §213.9 ✓）。

### 213.8 仍然开着的缺口

1. **C7 运维单点** ✓：只有用户能改变 ✓（第二个账号 ✓）。**连续六轮** ✓。
2. **GUI 面板本身没有被自动化测过** ✓（§211.11 第 3 条 ✓）。
3. **默认 socket 路径是全局的** ✓（§211.11 第 4 条 ✓）：两个工作区会撞 ✓，没有断言 ✓。
4. **附着时的取消** ✓：渲染刻意不走会话 ✓，所以在**附着**模式下 ✓，
   一次渲染仍然是**自己一个进程** ✓——**可以取消** ✓。而**如果**将来把渲染也接进附着 ✓，
   取消就只能是**请求级**的 ✓（发一条取消消息 ✓），**绝不能是杀进程** ✓。
   这条边界今天由配置的形状保证 ✓（`sessionActions` 里写渲染没有意义 ✓），**但没有断言** ✓。
...
## 214. 面板第一次有了判据：把「它说的是实话」与「它好看」分开

### 214.1 轮初读数：没有红（读完才动手）

契约层 **78/78** ✓、验收 **96 条 ✓**、`ALL SUITES PASSED` ✓。

### 214.2 缺口：这一族里唯一没有判据的部分

§213.8 第 2 条 ✓：Live Bridge 这一族（传输 ✓、协议 ✓、失败 ✓、附着 ✓）**每一块都有判据** ✓，
**只有面板没有** ✓——而面板是**用户唯一真的会看的东西** ✓。

### 214.3 把问题拆成两半，只自动化能自动化的那一半

**「它好看吗」需要一块屏幕和一个人** ✓——那是人的判断 ✓，**这一轮不假装能测它** ✓。
**「它说的是实话吗」不需要** ✓：面板的 `draw()` 只碰 `self.layout` ✓，
所以把**一个会记录的 layout** 交给它 ✓，它写下的每一行就是**它要显示的东西** ✓。

于是 `deepblend/tests/lib/bridge-panel-probe.py` ✓：在 Blender 的**无头模式**里
**真的注册这个插件** ✓、把面板画进记录器 ✓、并**真的通过 socket 问一次** ✓，
于是面板上那个计数**来自真实发生的事** ✓。
...
| 状态 | 面板写的 |
|---|---|
| 注册之前 ✓ | `Bridge is not running` ✓（**不是一片空白** ✓） |
| 注册之后 ✓ | `Status: listening on /tmp/…/panel.sock` ✓、`Socket: panel.sock` ✓、`Operations served: 0` ✓ |
| **真的服务过一次之后** ✓ | `Status: attached` ✓、`Operations served: 1` ✓、`Last: get_capabilities` ✓ |

**这一条是关键的** ✓：**计数来自发生的事** ✓——而不是一个常量 ✓
（变异实测 ✓：把它写成常量 ✓，红 ✓）。

### 214.4 断言，以及一句写下来的边界

`live-session.e2e.mjs` 36 → **42 项** ✓，六条新增 ✓：面板在没跑时**说话** ✓、
注册后**点名它监听的 socket** ✓、真的干过活之后**报出附着状态 / 计数 / 最后一次动作** ✓、
**计数随发生的事变** ✓、插件带着**用户必须填的那一行偏好设置** ✓（因为 Blender 会把插件单独复制走 ✓）、
以及**取消注册会把 socket 拿走** ✓（禁用插件之后不该还有东西在监听 ✓）。

**而这六条不证明的** ✓，套件头部**明写**了 ✓：**面板排得好不好看** ✓。
**说清一条判据不覆盖什么，是判据与主张的区别** ✓。

### 214.5 变异测试：四条，一条活了下来

| 变异 | 结果 |
|---|---|
| 计数写成常量 | 红 ✓ |
| 不再显示最后一次动作 | 红 ✓ |
| **不再单独显示 socket 那一行** | **活了下来** ✗ → 收紧后红 ✓ |
| 取消注册不拿走 socket | 红 ✓ |

**存活者又是那个形状** ✓（形状 **A** ✓）：那条断言问的是「面板文字里有没有 `listening on`」 ✓，
而**状态那一行本来就写着 `listening on <完整路径>`** ✓——**它包含了** ✓。
**被另一行满足的检查，不是对这一行的检查** ✓。
改成要求**那一行自己**（`^Socket: …\.sock$` ✓）之后 ✓，它当场红 ✓。

### 214.6 顺带修掉的两个探针自己的错

写这个探针时撞到两个 ✓，都记下来 ✓：

1. **`register()` 读的是环境变量** ✓，不是我给的 `--socket` 参数 ✓——
   于是桥起在了**默认路径**上 ✓，而探针去连我指定的那个 ✗。
   **修法不是让探针绕过它** ✓，而是**照用户的方式告诉它** ✓（设 `DEEPBLEND_BRIDGE_SOCKET` ✓）。
2. **协议的第一行是握手** ✓，不是答案 ✓——`ask()` 读第一行 ✓，
   于是把一次**成功**的请求报成「没回答」 ✓。**这正是这个协议的一个调用者会踩的坑** ✓。

### 214.7 收口读数

* 契约层 **78/78** ✓；`live-session.e2e.mjs` **42 项** ✓。
* 本轮**没有改 `packages/**`** ✓（新文件是测试资产 ✓，产品文件只在变异里被动过又还原 ✓），
  所以四步发布链**没有**触发 ✓。

### 214.8 仍然开着的缺口

1. **C7 运维单点** ✓：只有用户能改变 ✓（第二个账号 ✓）。**连续七轮** ✓。
2. **默认 socket 路径是全局的** ✓（§211.11 第 4 条 ✓）：两个工作区会撞 ✓，没有断言 ✓。
3. **附着时的取消** ✓（§213.8 第 4 条 ✓）：今天由配置的形状保证 ✓（`sessionActions` 里写渲染没有意义 ✓），
   而**没有断言** ✓。
4. **面板的「好看」仍然只有人看** ✓——**这是这一轮明确不覆盖的** ✓，写在套件头部 ✓，
   而不是留着让人以为已经测过 ✓。

---

## 215. 一个**什么都没做**的配置键：静默失效比报错更糟

### 215.1 轮初读数：没有红（读完才动手）

契约层 **78/78** ✓、验收 **96 条 ✓**、`ALL SUITES PASSED` ✓。

### 215.2 缺口：边界是真的，而它没有被写下来

§214.8 第 3 条 ✓。先把边界**量清楚** ✓：`runBootstrap` 是**唯一**决定走不走会话的地方 ✓，
而全文件只有**四个**调用点 ✓——`get_capabilities` ✓、`compile_scene` ✓、`render_preview` ✓、`render_views` ✓。
另外三个地方**自己 spawn** ✓：会话本身 ✓、**帧序列** ✓（`render_frames` ✓）、以及能力探测的冒烟渲染 ✓。

于是事实是 ✓：**`render_frames` 永远不可能被路由** ✓——
而**实测**：把它写进 `sessionActions` ✓ **什么都不会发生** ✓。
渲染照旧在自己的进程里跑 ✓，配置**静静地失效** ✓，**没有任何地方说过一个字** ✓。

**一个静静地什么都不做的键，比一个报错的键更糟** ✓：
**操作者以为他改变了什么** ✓。这正是这个仓库在配置面已经立过的规矩 ✓
（`assertKnownConfigKeys` 对「这一行不读的键」直接抛 ✓），而这里漏了一格 ✓。

### 215.3 修法：可路由集合是**推导**出来的，而不可路由的名字**启动即拒**

`SESSION_ROUTABLE_ACTIONS` ✓ 导出 ✓，而它的**来源是源码** ✓：
「凡是从 `runBootstrap` 走的动作」 ✓——**不是选出来的** ✓，是**数出来的** ✓。
`contract/session-routing.test.mjs` **双向**校验 ✓：
列表里多一个没有调用点的动作 → 红 ✓；调用点有而列表里少一个 → 红 ✓。

而**构造时拒绝** ✓：`sessionActions` 里出现不可路由的名字 ✓，抛出的消息点名**那个动作** ✓、
点名**为什么渲染被排除** ✓、并点名**杀掉会话在附着时会结束谁** ✓——**用户自己的 Blender** ✓。

### 215.4 变异测试：四条全红

| 变异 | 结果 |
|---|---|
| 可路由列表多一个没有调用点的动作 | 红 ✓（双向校验 ✓） |
| 可路由列表少一个调用点有的动作 | 红 ✓ |
| 不可路由的名字被接受而不是拒绝 | 红 ✓ |
| 帧序列改成走 `runBootstrap` | 红 ✓（**取消边界被移动了，而断言看见了** ✓） |

第四条是这一族里最该被盯着的一条 ✓：**`render_frames` 排除在外的理由**不是偏好 ✓，
而是**取消一次渲染必须恰好杀掉那一次** ✓；把它接进会话 ✓，取消就只能是**结束会话** ✓，
**而在附着的模式下那就是用户的 Blender** ✓。这条边界现在**移动就会被抓到** ✓。

### 215.5 收口读数

* 契约层 **79/79** ✓（本轮新增 `contract/session-routing.test.mjs` ✓，四项 ✓）；
  README 快照按真实值改过 ✓（78→**79** 个契约文件 ✓、95→**96** 个文件 ✓、96→**97** 条照跑 ✓、
  `node:test` 447→**453**（44→**45** 个文件）✓、自计断言 **1590** 不变 ✓、
  全新 clone 的读数 `16/78`→**`16/79`** ✓）。
  **`node:test` 那个数我第一版写成 451** ✗——**没有任何断言看着它** ✓，
  但**一个写下来的数字应该是对的** ✓，按工具的读数改成 453 ✓。
* 验收 **18 套件全绿** ✓。
* 本轮**改了 `packages/**`** ✓，所以四步发布链触发 ✓（见 §215.7 ✓）。

### 215.6 仍然开着的缺口

1. **C7 运维单点** ✓：只有用户能改变 ✓（第二个账号 ✓）。**连续八轮** ✓。
2. **默认 socket 路径是全局的** ✓（§211.11 第 4 条 ✓）：同一台机器上两个工作区会撞 ✓，没有断言 ✓。
3. **`render_preview` / `render_views` 在可路由列表里，而它们也是渲染** ✓——
   它们与 `render_frames` 的区别是**它们不可取消** ✓（一次性、短 ✓），
   所以它们进会话是安全的 ✓。**这个区别今天写在列表的注释里 ✓，但没有断言** ✓：
   如果哪天有人给它们加上取消 ✓，那条边界会**悄悄**变错 ✓。
4. **面板的「好看」仍然只有人看** ✓（§214.8 第 4 条 ✓）。

### 215.7 发布链

```
0. version 0.2.7 -> 0.2.8 ✓ + version:sync ✓
1. release:tarball ✓
2. gh release create v0.2.8 ✓
3. publish:packages -> 「还没有」 ✓（照它说的再跑 ✓）
4. result: all 7 packages are on the registry, each one read back ✓
5. release:parity -> problems: 0 ✓；两份探针日志按 0.2.8 重写 ✓
```

**第五次连着这样** ✓：那条消息说「还没有」 ✓、说「再跑一次」 ✓，照做就成了 ✓。
**一条读数能告诉读者下一步做什么，才算读数** ✓。

---

## 216. 「能取消」与「不能取消」：把一条注释变成一条规则

### 216.1 轮初读数：没有红（读完才动手）

契约层 **79/79** ✓、验收 **97 条 ✓**、`ALL SUITES PASSED` ✓。

### 216.2 缺口：一条**只写在注释里**的边界

§215.6 第 3 条 ✓：`render_preview` / `render_views` **在**可路由列表里 ✓，
而 `render_frames` **不在** ✓。区别不是「渲染 vs 不是渲染」 ✓，是**能不能取消** ✓——
一次性的预览渲完就返回 ✓，而帧序列**交出一个可杀的句柄** ✓。
**这条区别当时只写在列表的注释里** ✓：哪天有人给预览加上取消 ✓，
**那条边界会悄悄地变错** ✓，而没有任何东西会红 ✓。

### 216.3 修法：两边都**从源码推导**

**「能取消」= 那个 provider 方法交回一个可杀的 handle** ✓——**那个 handle 就是取消路径** ✓，
而**会话里的一次请求没有它** ✓。所以规则是 ✓：
...
* 交回 handle 的方法 ✓：全文件**恰好一个** ✓——`startFrameSequence` ✓（`return {` 后面跟着 `handle,` ✓）；
* 可路由的集合 ✓：上一轮已经是从 `runBootstrap` 的调用点数出来的 ✓。

于是**两个方向都会被看见** ✓：给一个可路由的动作**加上**取消 ✓ → 红 ✓；
把一个**能取消的**动作放进列表 ✓ → 红 ✓。**规则不再需要有人记得** ✓。

### 216.4 变异测试：三条，一条**是空操作**（而它指出了真洞）

| 变异 | 结果 |
|---|---|
| `render_preview` 长出可杀 handle（**写成一行**） | **活了下来** ✗ → 收紧后红 ✓ |
| 另一个方法也开始交回 handle（写成多行） | 红 ✓ |
| 能取消的动作被加进可路由列表 | 红 ✓ |

**第一条同时是两件事** ✓：我那条变异**写成了 `return { handle: null }` 一行** ✓，
而检查**只认多行那种写法** ✓（`return {\n handle,` ✓）——
于是变异**没有改到检查看得见的地方** ✓，而「没改到」与「活下来了」**长得一模一样** ✓（**第八次** ✓）。

**但它同时是一个真的洞** ✓：**一行写法的 `return { handle, … }` 会被漏掉** ✓。
**一个只认一种排版的样子检查，检查的是排版** ✓。改成认两种写法之后 ✓，
那条变异当场红 ✓（消息是「交回 handle 的方法集合变了」 ✓）。

### 216.5 顺带记一条量具的错

第一次写这条检查时 ✓，我用了一个 **2000 字符的窗口**去读 `startFrameSequence` 的方法体 ✓，
而动作名 `render_frames` **在窗口之外** ✓——于是断言说
「startFrameSequence 不再服务 render_frames」 ✓，**而它明明在服务** ✓。
**看错了切片的检查，会对正确的代码说错话** ✓；改成按**方法真实的范围**取（下一个 `async` 之前 ✓），它才通过 ✓。

### 216.6 收口读数

* 契约层 **79/79** ✓；`session-routing.test.mjs` 4 → **5 项** ✓。
* 本轮**没有改 `packages/**`** ✓（新断言在契约层 ✓），所以四步发布链**没有**触发 ✓。

### 216.7 仍然开着的缺口

1. **C7 运维单点** ✓：只有用户能改变 ✓（第二个账号 ✓）。**连续九轮** ✓。
2. **默认 socket 路径是全局的** ✓（§211.11 第 4 条 ✓）：同一台机器上两个工作区会撞 ✓，没有断言 ✓。
3. **`startFrameSequence` 的方法体是「交回 handle」的唯一判据** ✓——
   如果哪天出现**第二个**可取消的动作 ✓，那条断言会红并**要求**有人来看 ✓（这正是它该做的 ✓），
   但**「哪些动作能取消」这张表今天仍然只有一处** ✓（方法集合 ✓），没有对应的**动作名**表 ✓。
4. **面板的「好看」仍然只有人看** ✓（§214.8 第 4 条 ✓）。

---

## 217. 一个 socket 路径有**三个**状态，而它此前只被当成两个

### 217.1 轮初读数：没有红（读完才动手）

契约层 **79/79** ✓、验收 **97 条 ✓**、`ALL SUITES PASSED` ✓。

### 217.2 缺口：第二个 Blender **悄悄接管**了第一个的 socket，还报了成功

§216.7 第 2 条 ✓。先量 ✓（`two-bridges` 探针 ✓，同一个路径起两个桥 ✓）：

```
first bridge: reachable = connected   (pid 29013)
second bridge said: {"kind": "ready", "pid": 29014, "socket": "…/shared.sock"}
after the second started, the socket is reachable = connected
```

**读法** ✓：第二个桥**报 `ready`** ✓——而那个 socket 现在是**它的** ✓。
**第一个 Blender 从此不可达** ✓，而它**仍然以为自己正在监听** ✓。
**第二个报的是成功** ✓。这正是这个仓库反复付代价的那种失败 ✓：
**做了别的事，然后报成功** ✓。

### 217.3 修法：三个状态，而不是两个

一个已存在的 socket 路径只有两种可能 ✓，而**区别决定一切** ✓：

| 状态 | 该怎么办 |
|---|---|
| **存在 + 有人应答** ✓ | **拒绝启动** ✓，并点名**怎么给它自己的路径** ✓（`DEEPBLEND_BRIDGE_SOCKET` ✓ / 插件偏好设置 ✓ / 停掉另一个 ✓） |
| **存在 + 没人应答** ✓（进程死掉留下的文件 ✓） | **替换它** ✓——在这里拒绝会让用户**除了删掉一个他看不见的文件之外无路可走** ✓ |
| **不存在** ✓ | 绑定 ✓ |

**拒绝是一条结果，不是一个堆栈** ✓：无头入口把它打成**一行 JSON** ✓
（`{"kind": "error", "message": …}` ✓），退出码非零 ✓；插件那一半照旧把它写进面板 ✓。

**实测（修好之后）** ✓：第二个桥拒绝 ✓、消息点名 `DEEPBLEND_BRIDGE_SOCKET` ✓、进程退出 ✓，
而**第一个仍然是回答的那一个** ✓（`live-session.e2e.mjs` 断的就是**同一个 pid** ✓）。

### 217.4 断言与变异：五条断言，四条变异全红

新增五条 ✓：桥在它被给定的路径上服务 ✓；**第二个桥在活的路径上拒绝并点名出路** ✓；
**拒绝是一份可解析的文档而不是堆栈** ✓；**第一个桥仍然是同一个 pid 在回答** ✓；
**没人应答的文件会被替换** ✓（崩溃恢复不能丢 ✓）。

| 变异 | 结果 |
|---|---|
| 活的 socket 照样被 unlink（**原来的接管**） | 红 ✓ |
| 陈旧的 socket 文件永不替换（**丢掉崩溃恢复**） | 红 ✓ |
| 拒绝不再点名 `DEEPBLEND_BRIDGE_SOCKET` | 红 ✓ |
| 拒绝变回堆栈 | 红 ✓ |

第二条是这一轮**最该被盯着**的一条 ✓：三个状态里最容易修坏的是**中间那个** ✓——
为了让「有人应答」安全而顺手拒绝一切已存在的文件 ✓，**崩溃之后用户就再也起不来了** ✓。

### 217.5 顺带记一条写测试时撞到的

模拟「陈旧文件」时 ✗，我直接往那个路径 `writeFileSync` ✓——**失败** ✓：
那个路径上**仍然是一个 socket inode** ✓（刚被杀掉的桥留下的 ✓），**往 socket 文件里写东西不成立** ✓。
改成**先删掉再写一个普通文件** ✓——而那正是**崩溃留下的状态** ✓：**一个存在、而没人应答的路径** ✓。

### 217.6 收口读数

* 契约层 **79/79** ✓；`live-session.e2e.mjs` 42 → **47 项** ✓。
* 本轮**改了 `packages/**`** ✓，所以四步发布链触发 ✓（见 §217.8 ✓）。

### 217.7 仍然开着的缺口

1. **C7 运维单点** ✓：只有用户能改变 ✓（第二个账号 ✓）。**连续十轮** ✓。
2. **同一台机器上两个工作区仍然共用默认路径** ✓——现在的行为是**第二个拒绝并告诉你怎么分开** ✓，
   而**默认值本身仍然是一个全局路径** ✓。真正的修法（按工作区派生默认路径 ✓）
   需要插件知道工作区 ✗，而它今天不知道 ✓；**「拒绝」是诚实的中间态** ✓，不是终点 ✓。
...
## 218. 全局的 socket 默认值会指向**别人那个** Blender

### 218.1 轮初读数：没有红（读完才动手）

契约层 **79/79** ✓、验收 **97 条 ✓**、`ALL SUITES PASSED` ✓。

### 218.2 缺口：真正的修法需要插件知道工作区，而它不知道

§217.7 第 2 条 ✓。上一轮的中间态是「第二个桥拒绝并告诉你怎么分开」 ✓——
**那解决的是两个桥撞在同一个路径上** ✓，而没有解决**另一个**危险 ✓：

socket 路径有一个**全局默认值** ✓，所以同一台机器上两个工作区**可以指向同一个 Blender** ✓。
于是 B 工作区的产品会去驱动 **A 工作区的用户正在看的那个窗口** ✓，
**操作落进一个谁都没打算改的场景里** ✓——而**两边都觉得自己没错** ✓。
**这是这一族里最安静的一种错** ✓：没有任何一方行为异常 ✓。

### 218.3 修法：让桥**说出**它服务哪个工作区，产品**核对**它

* 桥可以被告知 ✓（`DEEPBLEND_BRIDGE_WORKSPACE` ✓ 或 `--workspace <路径>` ✓），
  并把这件事写进**握手** ✓（`{"kind": "ready", …, "workspace": …}` ✓），面板上也显示 ✓；
* 产品在附着时**核对** ✓：桥**声明了**一个工作区 ✓ 而**不是自己这个** ✓ → **拒绝** ✓，
  消息点名**两个路径** ✓ 以及**两边各自该怎么改** ✓（`DEEPBLEND_BRIDGE_SOCKET` / `sessionSocket` ✓
  与 `DEEPBLEND_BRIDGE_WORKSPACE` ✓）；
* 桥**什么都不声明**时 ✓ **照旧服务** ✓——那是这一条出现之前所有部署的行为 ✓，
  **向后兼容不是礼貌，是不让一个检查把现有部署弄坏** ✓。

**而拒绝不是杀** ✓：被拒绝的那个 Blender **继续活着** ✓（它是用户的 ✓），断言里有这一条 ✓。

### 218.4 断言与变异：四条断言，四条变异全红

新增四条 ✓：桥在握手里**声明工作区** ✓；服务**别的工作区**的桥被拒绝 ✓ 且**点名两个路径** ✓；
拒绝消息**说清两边各自怎么改** ✓；**被拒绝的 Blender 继续活着** ✓。
...
| 桥不再声明工作区 | 红 ✓ |
| 拒绝不再点名两个路径 | 红 ✓ |
| **什么都不声明的桥也被拒绝**（向后兼容丢掉） | 红 ✓ |

第四条是这一轮最该被盯着的一条 ✓：为了让「别的工作区」安全 ✓，
最顺手的写法是**拒绝一切不匹配** ✓——而 `null` 与「别的工作区」**不匹配** ✓，
于是**所有现存部署一起坏掉** ✓。这条边界现在有断言 ✓。

### 218.5 收口读数

* 契约层 **79/79** ✓；`live-session.e2e.mjs` 47 → **51 项** ✓。
* `install.md` 的插件一节多了这一条 ✓：**为什么要设** ✓（两个工作区可以指向同一个 Blender ✓）、
  以及**不设也不会坏** ✓。
* 本轮**改了 `packages/**`** ✓，所以四步发布链触发 ✓（见 §218.7 ✓）。

### 218.6 仍然开着的缺口

1. **C7 运维单点** ✓：只有用户能改变 ✓（第二个账号 ✓）。**连续十一轮** ✓。
2. **默认 socket 路径仍然是一个全局路径** ✓——这一轮让「指向别人的 Blender」**变成一个被拒绝的错误** ✓，
   而**没有让默认值按工作区派生** ✓。派生的前提是插件知道工作区 ✓，
   而它今天**只能被告知** ✓（不知道就不能猜 ✓——**猜错比不知道更糟** ✓）。
3. **`startFrameSequence` 的方法体是「交回 handle」的唯一判据** ✓（§216.7 第 3 条 ✓）。
4. **面板的「好看」仍然只有人看** ✓（§214.8 第 4 条 ✓）。
...
## 219. 工作区可以**读出来**，而不是只能被告知

### 219.1 轮初读数：没有红（读完才动手）

契约层 **79/79** ✓、验收 **97 条 ✓**、`ALL SUITES PASSED` ✓。

### 219.2 缺口：派生的前提是插件知道工作区，而它只能被告知

§218.6 第 2 条 ✓。上一轮让「指向别人的 Blender」**变成一个被拒绝的错误** ✓，
而**默认值本身仍然是全局的** ✓——因为派生需要一个前提 ✓：**插件得知道工作区** ✗，
而它当时**只能被告知** ✓。这一轮补的就是那个前提 ✓。

### 219.3 修法：**读**出来，而读不出来时**什么都不说**

**这个产品写的 checkpoint 就住在** `<工作区>/.deepblend/projects/<id>/revisions/<rev>/scene.blend` ✓——
所以一个**打开着**这种文件的 Blender ✓，**就是在那个工作区里干活** ✓：**路径自己说了** ✓。
于是插件从 `bpy.data.filepath` 里读它 ✓。

**而其余一切情况都返回空** ✓：空路径 ✓、用户自己做的文件 ✓、别的布局 ✓——
**空，就是产品当作「服务任何工作区」的那个值** ✓。

**这个不对称是故意的** ✓：一个**说不准却仍然声明**的桥 ✓，
会被产品因为**用户没有造成的**不匹配而拒绝 ✓，而用户会以为是自己配置错了 ✓，
**其实是一次猜错** ✓。**路径没说的时候，沉默是诚实的答案** ✓。

**优先级** ✓：**操作者显式设的永远优先** ✓（`DEEPBLEND_BRIDGE_WORKSPACE` / `--workspace` ✓），
派生**只用来填补沉默** ✓。

**实测**（真的把 checkpoint 打开 ✓）：

| 打开的文件 | 读出来的工作区 |
|---|---|
| `<repo>/.deepblend/projects/watch-commercial/revisions/r0002/scene.blend` ✓ | **`<repo>`** ✓ |
| `/tmp/derive-checkpoint.blend` ✓（同一个文件，拷到别处 ✓） | **空** ✓（**不猜** ✓） |
| `/tmp/not-a-workspace.deepblendX/projects/a.blend` ✓ | **空** ✓（标记要求**分隔符** ✓，不是子串 ✓） |
| 没有打开文件 ✓ | 空 ✓ |

### 219.4 断言、变异，以及一条**变异找出来的真缺口**

新增三条 ✓：打开着我们的 checkpoint 的 Blender **声明那个工作区** ✓；
**同一个部署不需要任何人配置就能被服务** ✓（派生只有喂给它检查的答案是对的才有用 ✓）；
**操作者点名的那个永远优先** ✓。
...
| 派生不再认识那个布局 | 红 ✓ |
| 派生对**任何**文件都猜一个工作区 | 红 ✓ |
| **派生盖掉操作者的显式设置** | 第一次 **NOT-APPLIED** ✗ → 补断言后红 ✓ |

**第三条同时是量具的问题和一个真缺口** ✓：
它第一次**没有应用** ✓（我的 shell 引号把多行模式弄坏了 ✓——**第九次** ✓），
而**去补它的时候发现**：前面那些检查**从来只跑过「没有显式设置」的桥** ✗——
**一个让派生盖掉操作者的版本，会通过当时所有的检查** ✗，
而设了 `DEEPBLEND_BRIDGE_WORKSPACE` 的人会被**静静地忽略** ✓——
**正是这份账本已经花了三轮在追的那种失败** ✓。补上「操作者优先」那条之后 ✓，它当场红 ✓。

### 219.5 收口读数

* 契约层 **79/79** ✓；`live-session.e2e.mjs` 51 → **54 项** ✓。
* `install.md` 的插件一节说清了**通常不用设** ✓、**什么时候必须设** ✓、以及**为什么读不出来时它不说** ✓。
* 本轮**改了 `packages/**`** ✓，所以四步发布链触发 ✓（见 §219.7 ✓）。

### 219.6 仍然开着的缺口

1. **C7 运维单点** ✓：只有用户能改变 ✓（第二个账号 ✓）。**连续十二轮** ✓。
2. **默认 socket 路径本身仍然是全局的** ✓：这一轮让**工作区**可以自动读出来 ✓，
   而**路径**仍然要人指定 ✓——「按工作区派生路径」需要一个**约定**（例如
   `<工作区>/.deepblend/bridge.sock` ✓），而**约定一旦写进文档就得有人守** ✓，
   今天没有 ✓。
3. **`startFrameSequence` 的方法体是「交回 handle」的唯一判据** ✓（§216.7 第 3 条 ✓）。
4. **面板的「好看」仍然只有人看** ✓（§214.8 第 4 条 ✓）。

### 219.7 发布链：上一轮的两条教训，这一轮都照做了

```
0. version 0.2.10 -> 0.2.11 ✓ + version:sync ✓
1. git push ✓（这次一次就成 ✓）
2. npm run release:tarball -> 退出码 **0** ✓（**读的是它自己的退出码** ✓，不是管道的 ✓）
3. gh release create v0.2.11 ✓ -> **读回来** ✓：资产 size 405528 ✓、`state: uploaded` ✓、
   而 URL 解出来的 `package.json` 是 **0.2.11** ✓
4. publish:packages -> 「还没有」 ✓（照它说的再跑 ✓）-> all 7 packages ✓
5. release:parity -> problems: 0 ✓；两份探针日志按 0.2.11 重写 ✓
```

**上一轮那两条教训各用了一次** ✓：**管道会藏起左边命令的失败** ✓（这一轮直接看退出码 ✓），
以及**发布链的每一步都要读回来** ✓（资产不是「传上去了」✓，是**解出来是 0.2.11** ✓）。

---

## 220. 约定：每个工作区一个 socket，以及**守它的人**

### 220.1 轮初读数：没有红（读完才动手）

契约层 **79/79** ✓、验收 **97 条 ✓**、`ALL SUITES PASSED` ✓。

### 220.2 缺口：约定一旦写进文档，就得有人守

§219.6 第 2 条 ✓。三件事现在都成立 ✓：**工作区读得出来** ✓、
**指向别人的 Blender 会被拒绝** ✓、**同一个路径上第二个桥会拒绝** ✓。
而**默认路径本身仍然是全局的** ✓——于是「两个工作区」这件事 ✓，
用户仍然要**自己动手**才能分开 ✓。

### 220.3 修法：socket 跟着工作区走

知道工作区时 ✓，桥默认监听 **`<工作区>/.deepblend/bridge.sock`** ✓——
**每个工作区一个** ✓，于是**两个工作区根本不会撞** ✓，不需要用户做任何事 ✓。
不知道工作区时 ✓，退回**机器级默认值** ✓（`~/.deepblend-bridge.sock` ✓），
也就是这条约定出现之前所有部署的行为 ✓。显式设的 `DEEPBLEND_BRIDGE_SOCKET` **永远优先** ✓。

**路径在 `.deepblend/` 里** ✓，而那是 store 目录 ✓、**`.gitignore` 第 32 行** ✓——
**socket 是运行时的东西，不是源文件** ✓，它**永远不该可提交** ✓（这一条有断言 ✓）。
顺带一个实现细节 ✓：`bind` **不建目录** ✓，所以一个从没跑过产品的工作区里 ✓，
`.deepblend/` 可能还不存在 ✓——绑定前先建 ✓。

**实测** ✓：知道 `/w` → `/w/.deepblend/bridge.sock` ✓；什么都不知道 → `~/.deepblend-bridge.sock` ✓；
而一个真的开着 checkpoint 的 Blender ✓ 监听在 `/Users/hxb/workspace/deep-blend/.deepblend/bridge.sock` ✓。

### 220.4 守约的人：**文档说的路径与代码算出来的路径必须相同**

缺口自己写着这一句 ✓：**约定一旦写进文档就得有人守** ✓。所以新增的断言里有一条 ✓，
把 **`install.md` 里写的那句话** ✓ 与 **代码算出来的相对路径** ✓ 比在一起 ✓——
...
| 变异 | 结果 |
|---|---|
| 约定挪出 store（socket 变成可提交的路径） | 红 ✓ |
| 什么都不知道的桥也拿到工作区形状的路径 | 红 ✓ |

**第二条第一次报的失败信息不是为它写的那条** ✗（套件按顺序跑 ✓，先撞上了别的 ✓）——
于是**手工验了一遍** ✓：把变异应用上 ✓、直接问探针 ✓ → `withoutWorkspace` 变成
`/tmp/.deepblend/bridge.sock` ✓、而机器默认值没变 ✓ → **为它写的那条断言确实会红** ✓。
**一条被"别的断言"杀死的变异，只证明它弄坏了什么** ✓，不证明**为它写的那条**有效 ✓。

### 220.6 顺带记一条这一轮自己的错

写完断言之后我**以为**脚本跑过了 ✓——它其实**只是被 `cat` 写了出来** ✓，**从来没被执行** ✗，
而我把上一条命令的输出当成了它的 ✓。发现的方式是**下一步 `grep` 找不到那段代码** ✓：
**「我以为它跑了」不是证据，文件里有没有那段代码才是** ✓。

### 220.7 收口读数

* 契约层 **79/79** ✓；`live-session.e2e.mjs` 54 → **58 项** ✓。
* `install.md` 的插件一节写下了这条约定 ✓，以及**不知道工作区时会发生什么** ✓。
* 本轮**改了 `packages/**`** ✓，所以四步发布链触发 ✓（见 §220.9 ✓）。

### 220.8 仍然开着的缺口

1. **C7 运维单点** ✓：只有用户能改变 ✓（第二个账号 ✓）。**连续十三轮** ✓。
2. **产品侧不会去猜那个约定路径** ✓：`sessionSocket` 仍然要显式设 ✓——
   产品**不去 `<工作区>/.deepblend/bridge.sock` 试着连一下** ✓。
   这是**刻意的** ✓（猜错比不知道更糟 ✓，与工作区那一条同源 ✓），
   但它意味着**用户仍然要把这个路径写进配置** ✓——文档给了它 ✓，产品没有替你写 ✓。
3. **`startFrameSequence` 的方法体是「交回 handle」的唯一判据** ✓（§216.7 第 3 条 ✓）。
4. **面板的「好看」仍然只有人看** ✓（§214.8 第 4 条 ✓）。
...
## 221. 发现，而不是猜：产品自己去约定位置找桥

### 221.1 轮初读数：没有红（读完才动手）

契约层 **79/79** ✓、验收 **97 条 ✓**、`ALL SUITES PASSED` ✓。

### 221.2 缺口：约定写下来了，而产品不肯用它

§220.8 第 2 条 ✓。上一轮立了约定（`<工作区>/.deepblend/bridge.sock` ✓），
而**产品不会去那儿找** ✗——用户仍然得把这个路径**抄进配置** ✓。
那条「不猜」的理由是**对的** ✓（**猜错比不知道更糟** ✓），
但它把**两件不同的事**混成了一件 ✓：

* **猜** = **断言**它在那儿 ✗ → 猜错就是一次失败 ✗；
* **发现** = **先看**那儿有没有东西应答 ✓ → **有就用** ✓，**没有就照旧** ✓。

### 221.3 修法：先看再退，而**退回去是设计的一部分**

`sessionSocket` 没设时 ✓，产品去看 `<工作区>/.deepblend/bridge.sock` ✓：

| 那儿的状态 | 产品做什么 |
|---|---|
| **有东西应答** ✓ | 附着上去 ✓（`_sessionOrigin: 'discovered'` ✓）——而**工作区核对照样适用** ✓，所以发现**不可能**接到别人工作区的 Blender 上 ✓ |
| **文件在、没人应答** ✓（崩溃留下的 ✓） | **照旧自己起一个** ✓（`'spawned'` ✓）——**不是失败** ✓ |
| **什么都没有** ✓ | 同上 ✓（**这条约定出现之前的行为** ✓） |

显式设了 `sessionSocket` 的 ✓，**永远优先** ✓（`'configured'` ✓）。
而**「这一次用的是哪一个」被记下来** ✓（`_sessionOrigin` ✓）——
「这个部署到底在驱动用户的 Blender 还是它自己的」 ✓，
是 Live Bridge 行为异常时**第一个要问的问题** ✓，而**唯一知道答案的地方就是做选择的那一支** ✓。

### 221.4 一次意外，验证了整个设计

测试第一次跑**失败**了 ✓，而失败信息（**我把桥自己的输出挂在了失败详情里** ✓）写着：

```
{"kind": "error", "message": "OSError: AF_UNIX path too long"}
```

**Unix socket 路径有长度上限** ✓（macOS 上约 104 字节 ✓），
而这个套件自己的临时目录**足够长** ✓，于是 `<工作区>/.deepblend/bridge.sock` 超限 ✓。
**桥拒绝了** ✓（干净的一行 JSON ✓），而**产品退回去自己起了一个** ✓、**照样把活干完** ✓——
**这正是「发现」相对「猜」的全部价值** ✓：一个不存在的桥**不是一次失败** ✓，
而是**回到这条约定出现之前的行为** ✓。

于是新增一条断言 ✓（工作区路径深到超限时**仍然能用** ✓、`'spawned'` ✓），
而 `install.md` 写下了这个平台限制与两条出路 ✓。

### 221.5 变异测试：三条，一条**活了下来**——而它指出了真洞

| 变异 | 结果 |
|---|---|
| 发现根本不去看那个位置 | 红 ✓ |
| **发现失败时直接失败，而不是退回** | **活了下来** ✗ → 补断言后红 ✓ |
| 显式配置被忽略、一律走发现 | 红 ✓ |

**第二条为什么活着** ✓：我当时那条「路径太深」的断言 ✓ **根本走不到发现** ✗——
桥拒绝之后**没有留下文件** ✓，于是 `existsSync` 是假 ✓、**发现从未被尝试** ✓。
**它测的是「没有文件 → 自己起一个」** ✓，而那一条**第一个断言已经测过** ✓。

**真正会走到退回的那一格**是 ✓：**文件在、而没人应答** ✓——**崩溃留下的状态** ✓。
补上它之后 ✓，那条变异当场红 ✓。

**顺带修了一处测试自己的不体面** ✓：那条变异让 `runBootstrap` **抛出去** ✓、
**整个套件中止** ✗——变异是被抓到了 ✓，但读者看到的是一段堆栈 ✓、
**而后面每一条检查都静静地没跑** ✓。把抛出接成结果之后 ✓，
它现在报的是**那条检查自己的名字** ✓。

### 221.6 收口读数

* 契约层 **79/79** ✓；`live-session.e2e.mjs` 58 → **65 项** ✓。
* `install.md` 的插件一节写下了**发现**（不是猜 ✓）、**你写的永远优先** ✓、
  以及那个**平台长度限制**与两条出路 ✓。
* 本轮**改了 `packages/**`** ✓，所以四步发布链触发 ✓（见 §221.8 ✓）。

### 221.7 仍然开着的缺口

1. **C7 运维单点** ✓：只有用户能改变 ✓（第二个账号 ✓）。**连续十四轮** ✓。
2. **发现只认一个位置** ✓：约定路径 ✓。`DEEPBLEND_BRIDGE_SOCKET` 能让桥听在别处 ✓，
   而**产品不会去那些地方找** ✓——这是刻意的 ✓（一份可枚举的搜索路径 ✓ 比一张地图好维护 ✓），
   但它意味着**挪了地方就得写进配置** ✓。
3. **`startFrameSequence` 的方法体是「交回 handle」的唯一判据** ✓（§216.7 第 3 条 ✓）。
4. **面板的「好看」仍然只有人看** ✓（§214.8 第 4 条 ✓）。
