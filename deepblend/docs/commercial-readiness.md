...
- **移动：M5** — 关闭 §220.8 的第 2 条 ✓：**产品自己去约定位置找桥，而不是等用户抄路径** ✓。
  关键是把**两件不同的事**分开 ✓：**猜**是**断言**它在那儿 ✗（猜错就是失败 ✗），
  而**发现**是**先看**那儿有没有东西应答 ✓——**有就用** ✓、**没有就照旧** ✓。
  三种状态各有去处 ✓：有应答 → 附着 ✓（而**工作区核对照样适用** ✓，所以发现接不到别人工作区的 Blender ✓）；
  文件在而没人应答（崩溃留下的 ✓）→ **照旧自己起一个** ✓，**不是失败** ✓；什么都没有 → 同上 ✓。
  显式配置**永远优先** ✓，而「这一次用的是哪一个」被记下来 ✓（`_sessionOrigin` ✓）——
  **唯一知道答案的地方就是做选择的那一支** ✓。
- **移动：M4** — 一条变异**活了下来** ✓，而它指出了真洞 ✓：
  「发现失败时直接失败、不退回」 ✓——因为我那条「路径太深」的断言**根本走不到发现** ✗
  （桥拒绝后**没有留下文件** ✓）。**真正会走到退回的那一格**是**文件在而没人应答** ✓——
  **崩溃留下的状态** ✓；补上它之后 ✓，变异当场红 ✓。
  顺带把测试自己的不体面修了 ✓：那条变异让 `runBootstrap` 抛出去、**整个套件中止** ✗，
  **后面每条检查都静静地没跑** ✓；现在它报的是**那条检查自己的名字** ✓。
- **移动：M6** — 一个可数量化读数被记录 ✓：`live-session.e2e.mjs` 从 **58 项** ✓ 到 **65 项** ✓。
- **一次意外验证了设计** ✓：套件的临时路径长到超过 Unix socket 上限 ✓（约 104 字节 ✓），
  桥干净地拒绝 ✓、而产品**退回自己起一个、照样把活干完** ✓——
  **一个不存在的桥不是一次失败** ✓，而是回到这条约定出现之前的行为 ✓。
  平台限制与两条出路写进了 `install.md` ✓。
- **判据**：`node deepblend/tests/blender-integration/live-session.e2e.mjs` ✓（65 项 ✓）、
  `packages/deepblend/provider-local/lib/index.js` 的发现分支与 `_sessionOrigin` ✓。
- **还差什么**：账本上仍然开着的是 C7 与 C15 ✓。
  本轮改了 `packages/**` ✓，所以四步发布链触发 ✓（见 §221.8 ✓）。
  下一轮的第一顺位是 **§221.7 的第 2 条** ✓：**发现只认一个位置** ✓——
  刻意的 ✓（一份可枚举的搜索路径比一张地图好维护 ✓），但它意味着挪了地方就得写进配置 ✓；
  **C7** 只有用户能改变 ✓，**连续十四轮** ✓。
