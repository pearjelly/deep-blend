...
**这条事实也落在 job 记录上**，因为任务输出会被先读到它的人取走、进程重启之后就没了：
`blender_job_status` 会多打一行

```
warning:  [JOURNAL_INCOMPLETE] an attempt at this job was cut off mid-line in its event journal …
```

看到它不需要做任何事——它说的就是「这里的数字不是从这个日志数出来的」。它会一直留在
记录里（记录是历史），所以一个已经续渲完成的 job 也可能带着它，那表示**某一次尝试**被截断过。
**你主动取消的渲染既不会有这一行、也不会有上面那句输出**：`cancelJob` 是故意杀进程组的，
日志断在半行是那个动作的预期结果，不是发现。

这条诊断自己有一条规则，四个条件，测试在 `deepblend/tests/contract/render-journal.test.mjs`：
**文件断在半行 × 渲染器确实已经停下 × 不是你主动杀的 × 还没说过**。
轮询是每秒一次，一个活着的渲染器「最后一行还没写完」是常态——把它报成被杀，
就是每次渲染都来一次的假警报。四个条件写在 `incompleteJournalWarning()` 里而不是调用点，
原因是一条**实测**：真的 SIGKILL 几乎总是落在行边界上（第 25 轮量了两次，两次都干净），
所以「断了因而被记下」这个方向在端到端套件里**碰不出来**，那个分支就等于没人检查过。
它落在记录上的那一半由 M3 套件断言：**被 SIGKILL 的那次渲染**，文件是不是断在半行
和记录里有没有这一行**必须一致**（两个方向都能失败）；**被主动取消的那次**则必须没有。
