...
| **Python 3** | 只有一处：`contract/render-job.test.mjs` 用普通 CPython 跑 `deepblend_util.py`，比对两边的帧命名 | 那**一条**失败并说清缺什么，其余 93 条照跑 |
...
DeepBlend tests: 14/76 file(s) passed
```
...
预期：**17 个套件、92 个文件**全部通过。其中契约层（`run.mjs`，不需要 Blender）是
**76 个文件 = 1590 项自计断言（34 个文件打印计数）+ 438 个 `node:test` 用例（42 个文件）**。
